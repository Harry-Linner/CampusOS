...
本轮真实上游验收中，统一认证、教务会话和全部 8 次课表请求连续两轮得到成功反馈；综合验收随后都在同一门课程资料 activities 请求处超时，因此下一学年课表链路通过，但整套认证/教务/资料综合链路不得标记为全通过。
**Tier:** T3 · S3
...
2026-09-06 的校园资讯决议把“内容可见性”和“打扰策略”分开：资讯页始终保留抓取到的完整列表；每源通知开关与全局 OR 关键词只决定新增或标题/摘要更新是否进入通知中心和桌面提示。关键词为空时不筛选，首次成功抓取静默建基线，设置变更不补推历史。一次刷新批次在通知中心逐条保存，但桌面只显示一次汇总提示。
...
已验证账号的正式 workspace 不再使用 mock fixture：主进程刷新官方 connector，按当前认证账号读取 capability repository，再将课程、考试、有可信绝对时间的作业和课件目录投影到工作区。作业与资料目录每轮独立获取和降级；资料发布前必须完成学期、全部课程分页及每门课程 activities/uploads，禁止用部分成功覆盖上次完整快照。2026-07-29 使用 `live-auth.env` 的本科真实账号脱敏验证通过 ZJUAM、教务、素拓、全部课表学期、考试、成绩、`/api/todos`、学期、全部课程分页与逐课 activities 结构，以及一份授权私有课件的认证下载和实际字节校验，且没有输出敏感内容。认证下载 transport、reference → preview 回退、一次重认证、5 次指数退避、Range 和文件大小变化均由自动化协议测试覆盖；完整多设备流程与全新 Windows 安装验收仍未完成。
...
2026-08-03 的产品边界进一步确认：插件必须是左侧栏中用户可启停的完整模块，官方插件保留“学业、日程、资料”三个；2026-08-08 的受控提取决策将 AI 助手作为第四个官方模块加入。数据源协议适配改为 Core 托管连接器，事件投影、排程、搜索、通知和导出改为内部服务；它们继续使用版本化 capability contract，但不再作为插件向用户展示。该结论取代“每个 connector/feature 都是独立插件”的早期分类，不改变已验证的认证、请求、缓存、降级和解析流程。
...
- **Wedge:** ZJU 工科生 → 桌面端 VS Code 式工作台 → Core 教务连接器 + 学业/日程/资料/AI 助手四个模块。先让 100 个 CC98 技术板块用户成为核心用户和首批插件贡献者，再向普通学生扩散，最后向其他高校横向扩展。
...
The independent major endpoint projection is now retained as `majorSummary` in `academic.grades@1`, so the default major GPA does not silently become a recomputation over the transcript subset. The renderer does not introduce a local custom-weight recalculation path.
...
`.tmp/DeskToDo` 的有效可迁移经验集中在桌面常驻效率层，而不是 GitHub Gist 同步。历史对照曾记录桌面日历直接操作、无日期待办、托盘验收、时钟、天气、通用倒计时、进度条、组件启用/排序/配置、多显示器布局记忆，以及农历/节日/法定假期/外观配置；其中颜色优先级和无日期待办已明确不迁入 CampusOS。CampusOS 的校园连接器、资料下载、插件安全边界和提醒链不被 DeskToDo 流程替代。桌历需求决策见 [docs/desk-calendar-decisions.md](docs/desk-calendar-decisions.md)（旧的 DeskToDo 差异闭环计划文件已删除）。
...
- `.tmp/celechron-1.3.0/lib/http/ugrs_spider.dart` 表明本科课表并非只查当前与下一学年：它从学号推导入学年，抓取入学年至当前学年，并在毕业边界内探测下一学年；每学年顺序固定为秋、冬、春、夏。`2|夏` 即短学期；CampusOS 在学业课表中将其并入春夏学期并提供“只看短学期”，在资料模块中作为独立学期分组。
