...
课表尚不能直接进入日期日历。`zju-calendar-config` 已从浙江大学官方 HTTPS 校历页 `https://www.zju.edu.cn/english/19600/list.htm` 提取学季边界和开课日，并以 capability 驱动当前周或下一学季状态；但教务课表只给出学季、周几、节次和单双周，公开结构化来源未提供可信的本科节次钟点，教务处发布的 2026—2027 校历 PDF 又是图片型文件。下一项关键验证是找到稳定、可校验的节次时间与节假日调补机器源；在此之前不允许按固定日期、当前周或假定钟点强制展开课程。
