...
本工作区的任务分流、模型和推理强度选择见根目录 [`模型调用速览.md`](模型调用速览.md)。除非用户在当前任务明确指定，按其中的默认分工与升级条件执行；运行时模型可用性、额度和速度以实际界面为准。
...
#### Feature completion self-check

每轮实现必须在交付前做一次逐项自查：对照需求清单确认代码入口、正式数据/IPC/持久化链路、用户可见行为、错误边界和针对性测试都已完成。只要任一项没有真正完成，不得用按钮、占位、静态结果、mock 成功或文档描述宣称完成；必须继续修改和验证，或明确记录阻塞原因与未完成项。自查结果应同步写入对应功能 spec 的“自查记录”，并以实际命令输出作为验证证据。
