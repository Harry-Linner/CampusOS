...
- 首次引导已加入后台启动与桌面通知偏好，设置页可持续修改；通知中心以 `unread/read/handled/expired` 保存 30 天、最多 500 条并优先保护未读。日程提醒在触发时进入中心，启动补收最近 24 小时；校园资讯首次同步静默建基线，后续更新以可预览、可定位的轻量引用进入中心；下载完成仅作临时 toast 与音效。
