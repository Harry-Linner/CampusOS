...
CampusOS 的插件是用户可在扩展页启用、禁用，并在左侧栏获得恰好一个一级入口的完整功能模块。官方插件收敛为四个：**学业**（课表、课程、考试、成绩/GPA、素拓实践）、**日程**（日历、DDL、任务、自动排程、系统日历/iCal）、**资料**（课程资料与下载队列）和 **AI 助手**（显式消息提取与确认后写入日程）。总览、扩展、设置、全局搜索、开机自启、系统托盘和后台生命周期属于 Core。
