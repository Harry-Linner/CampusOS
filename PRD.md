...
- **日历 + 提醒系统** — 月历、线性日程、单日时间线、桌面系统通知、课程/作业/考试统一展示与可点击详情。课程、考试和上游作业只读；自建任务可从日历详情编辑、完成或删除，并可覆盖全局设置配置不提醒、开始/截止时、预设提前量或自定义时间。短学期在学业课表中继续属于春夏学期并提供“只看短学期”筛选，在资料模块中单独分组。课表事件只投影校历确定的当前完整学期；Celechron 对照的最近学期回落仅保留 14 天，之后休课期投影下一完整学期并在总览预览下一学期首次出现的同星期课程，真实课程日期不被改写。DDL 在上海自然日早于今天时不再进入待办或提醒。MVP 先把桌面场景下的"尽量不漏事"做到可用，再由 post-MVP 安卓端补齐离开电脑后的最后一公里提醒。
- **桌面后台能力** — Core 统一管理开机自启、系统托盘、关闭窗口行为、刷新和提醒调度。桌面日历只是日程插件功能；日程插件禁用时必须关闭该窗口并移除对应托盘项，退出 CampusOS 时所有窗口和后台任务一起停止。
