...
**Owner:** Harry-Linner
**Currency:** CNY (¥)
**Related docs:** [research](research.md) · [plan](plan.md) · [技术规格](docs/specs/campusos-interface-v3.md)

### 当前范围口径（2026-09 文档清理修订）
...
- 领域术语与运行时决策单一事实源：CONTEXT.md。
- 桌面日历始终位于桌面之上、普通应用之下，保留鼠标/键盘交互，不提供置顶模式。系统输入验收必须包含原生鼠标命中，不能仅用 CDP。重复事件与通知的追加修复及验收边界见 [审查记录](docs/audits/2026-09-05-desktop-and-schedule.md)。通知正文和状态也以本地 SQLite 为准，旧文件只导入一次。

### 插件模块边界（2026-08-03）

CampusOS 的插件是用户可在扩展页启用、禁用，并在左侧栏获得恰好一个一级入口的完整功能模块。官方插件为五个：**学业**（课表、课程、考试、成绩/GPA、素拓实践）、**日程**（日历、DDL、任务、系统日历/iCal；自动排程已于 2026-08-22 决议删除）、**资料**（课程资料与下载队列）、**AI 助手**（显式消息提取与确认后写入日程）和 **校园资讯**（campus-feed，抓取信息源 + AI 受控抽取转日程）。总览、扩展、设置、全局搜索、开机自启、系统托盘和后台生命周期属于 Core。

本科/研究生教务、学在浙大、素拓和在线校历是 Core 托管的数据连接器，不是插件，不出现在扩展列表或左侧栏。事件投影、任务存储、通知、诊断和系统导出是内部服务。移动端专属功能不进入桌面端产品范围。该边界以 [ADR-0002](docs/adr/0002-user-facing-plugin-modules.md) 和 [模块设计](docs/design/celechron-inspired-plugin-suite.md) 为准。
...
**开发数据范围（2026-08-16 更新）：** 课表抓取与日历投影继续以真实 `2026-2027 秋冬` 为最低正确性基线，但本科课表连接器按 Celechron 1.3.0 从学号推导入学年，依次抓取入学年至当前学年的秋、冬、春、夏，并额外探测不超过毕业边界的下一学年；`2|夏` 作为短学期，在学业课表中归入春夏学期，在资料模块中作为独立“短学期”分组展示。资料视图和新建下载任务支持学在浙大返回的全部历史学期，真实 `2025-2026 春夏` 仅保留为私有下载验收基线，不再作为 UI 投影白名单。私有基线只能位于 Git ignored 的本地目录，不进入 Git、CI、构建、日志或截图；完整约束见 [开发数据基线](docs/development-data-baselines.md)。

### Core user flow

1. 下载安装 CampusOS Windows 包 → 启动 App
2. 5 步向导：欢迎 → 输入 ZJU 教务账号 → 自动拉取课表（预览确认）→ 推荐官方插件 → 进入主页；结束时额外询问是否允许“登录 Windows 后自动启动 CampusOS”，默认不启用
3. 主页：固定 Core 导航（总览/扩展/设置）+ 已启用插件入口（学业/日程/资料/校园资讯/AI 助手）+ 主内容区；总览聚焦今日课程时间线与待办，休课期明确预览下一学期同星期课程，日程插件提供月历、周视图、线性日程、单日时间线和任务（自动排程已删除，见文件头当前范围口径）
...
- **官方模块（按 Celechron 对照迁移，现为五个）** — “学业”整合课表、课程、考试、成绩/GPA 和素拓实践；“日程”整合日历、DDL、个人任务、系统日历和 iCal（自动排程已于 2026-08-22 决议删除）；“资料”整合课程资料和下载队列；“AI 助手”做显式消息的受控抽取（ADR-0004）；“校园资讯”聚合校内外信息源并支持 AI 受控抽取转日程。数据连接器和内部服务由 Core 托管，继续通过细粒度 capability 协作。完整边界与迁移表见 [模块设计](docs/design/celechron-inspired-plugin-suite.md)。
...
- **日历 + 提醒系统** — 月历、线性日程、单日时间线、桌面系统通知、课程/作业/考试统一展示与详情。所有事件可双击编辑：本地事件开放完整字段；课程、考试和上游作业的正式字段只读，只允许保存本机个人备注与提醒。重复事件支持每 N 天、每 N 周并选星期、每 N 月、每 N 年，结束条件为永不/日期/次数，实例编辑范围为仅本次/本次及未来/整个系列，实例完成和删除状态独立。所有本地事件必须有起止时间，不提供无日期待办。短学期在学业课表中继续属于春夏学期并提供“只看短学期”筛选，在资料模块中单独分组。课表事件只投影校历确定的当前完整学期；Celechron 对照的最近学期回落仅保留 14 天，之后休课期投影下一完整学期并在总览预览下一学期首次出现的同星期课程，真实课程日期不被改写。DDL 在上海自然日早于今天时不再进入待办或提醒。
- **桌面后台能力** — Core 统一管理开机自启、系统托盘、关闭窗口行为、刷新和提醒调度。桌历的“随 CampusOS 开机恢复”从属于全局开机自启：全局关闭时强制关闭且不可开启。桌历状态与个性化数据写入 SQLite；日程插件禁用时关闭该窗口并移除对应托盘项，退出 CampusOS 时所有窗口和后台任务一起停止。
...
- 首批 Core 数据连接器：本科/研究生教务、在线校历、学在浙大、素拓；首批官方插件为 `学业`、`日程`、`资料`、`AI 助手`、`校园资讯` 五个完整左侧栏模块（口径更新见文件头"当前范围口径"）
- 当前已验证接入源：教务处网站、学在浙大、素拓、在线校历，以及 campus-feed 抓取面（源清单见 docs/campus-feed/zju-sources-guide.md）。计算机学院院网、云峰学院院网和 ETA 三全育人平台为从未接入的计划占位，不属 MVP 接入源
...
- CampusOS 生命周期已统一：主窗口关闭支持每次询问、隐藏到托盘或退出；支持“设为默认，以后不再询问”；单实例重复启动会恢复主窗口；托盘提供桌面日历月/周/日视图，日程插件停用时关闭桌面日历并移除对应控制。
- 首次引导已加入后台启动与桌面通知偏好，设置页可持续修改；通知中心以 `unread/read/handled/expired` 保存 30 天、最多 500 条并优先保护未读。日程提醒在触发时进入中心，启动补收最近 24 小时；校园资讯首次同步静默建基线，后续新增或标题/摘要更新先经过来源通知开关与全局关键词筛选，再以可预览、可定位的轻量引用进入中心；筛选不影响资讯页完整浏览，下载完成仅作临时 toast 与音效。
...
- 恢复过期实例必须经过用户确认；只恢复任务实例，不恢复已过期提醒，也不补发提醒。重复规则使用稳定实例标识，支持每 N 天、每 N 周并选择星期、每 N 月和每 N 年，以及永不/日期/次数结束和三种系列编辑范围。
- 主程序更新保持手动下载和安装：退出应用不会自动安装已下载版本；插件包沿用签名校验、隔离安装和失败回滚边界。
- 自建任务的单项提醒已接入正式存储与 Electron 调度链；插件更新已接入可信清单和按插件批准的下载/原子安装链，默认清单位于 `plugins/updates.json`。


## 2026-08-16 implementation status

- [x] App feedback entry uses a fixed GitHub Issues template without attaching private data.
- [x] Anonymous analytics consent is surfaced in first-run onboarding and Settings, defaulting to off.
- [x] DingTalk import/login is represented by a disabled, explicit placeholder.
- [ ] Cookie/browser-extension fallback login remains blocked on real upstream validation and is not exposed as a working login path.
