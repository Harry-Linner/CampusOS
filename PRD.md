...
主窗口首次关闭时询问“隐藏到托盘”或“退出 CampusOS”，并提供“设为默认，以后不再询问”。未勾选时本次选择只执行一次，下次继续询问；设置页持续提供“每次询问 / 隐藏到托盘 / 退出 CampusOS”三种行为。托盘负责重新打开主窗口、控制日程插件的桌面日历、立即同步和明确退出。退出 CampusOS 时，主窗口、托盘、桌面日历、刷新与提醒调度一起停止。完整决策见 [ADR-0005](docs/adr/0005-desktop-background-lifecycle.md)。
...
**开发数据范围（2026-08-16 更新）：** 课表抓取与日历投影继续以真实 `2026-2027 秋冬` 为最低正确性基线，但本科课表连接器按 Celechron 1.3.0 从学号推导入学年，依次抓取入学年至当前学年的秋、冬、春、夏，并额外探测不超过毕业边界的下一学年；`2|夏` 作为小学期并入春夏学期展示。资料视图和新建下载任务支持学在浙大返回的全部历史学期，真实 `2025-2026 春夏` 仅保留为私有下载验收基线，不再作为 UI 投影白名单。私有基线只能位于 Git ignored 的本地目录，不进入 Git、CI、构建、日志或截图；完整约束见 [开发数据基线](docs/development-data-baselines.md)。
...
- **日历 + 提醒系统** — 月历、线性日程、单日时间线、桌面系统通知、课程/作业/考试统一展示与可点击详情。课程、考试和上游作业只读；自建任务可从日历详情编辑、完成或删除，并可覆盖全局设置配置单项提醒。小学期继续属于春夏学期，在存在 `2|夏` 课程时显示“春夏学期（含小学期）”并提供“仅看小学期”筛选。课表事件只投影校历确定的当前完整学期；Celechron 对照的最近学期回落仅保留 14 天，之后休课期投影下一完整学期并在总览预览下一学期首次出现的同星期课程，真实课程日期不被改写。DDL 在上海自然日早于今天时不再进入待办或提醒。MVP 先把桌面场景下的"尽量不漏事"做到可用，再由 post-MVP 安卓端补齐离开电脑后的最后一公里提醒。
