...
const installBridge = (
  ...
  window.campusos = {
    ...
    downloads: {
      ...
      verify: vi.fn(async () => ({ status: "missing" as const, actualBytes: null, expectedBytes: null })),
      clearHistory: vi.fn(async () => 0),
      getPreferences: vi.fn(async () => ({ completionSound: true })),
...
describe("OnboardingWizard", () => {
  it("renders the welcome step on first mount", async () => {
    ...
    expect(screen.getByText("把所有校园生活工具集成在一起")).toBeDefined();
  ...
  it("requires authentication and never offers a development skip button", async () => {
    installBridge(vi.fn(async () => verifiedRecord));
    const onComplete = vi.fn();
    ...
    fireEvent.click(screen.getByRole("button", { name: "开始配置" }));
    const connect = await screen.findByRole("button", { name: "连接并保存" });
    expect(connect.hasAttribute("disabled")).toBe(true);
    expect(screen.queryByRole("button", { name: "开发模式跳过认证" })).toBeNull();
  ...
  it("completes onboarding and calls onComplete", async () => {
    ...
    expect(screen.getAllByText("待启用")).toHaveLength(3);
