...
describe("schedule event ranges", () => {
  ...
  it("filters events by kind via the type toggles", async () => {
    ...
    // 默认全部显示。
    expect(screen.getAllByText("Read notes").length).toBeGreaterThan(0);
    // 隐藏"我的日程"后，Read notes（本地任务事件）不再显示。
    fireEvent.click(screen.getByRole("button", { name: "我的日程" }));
    await waitFor(() => {
  ...
  it("不再把 snapshot.deadlines 里没有对应事件的项投影成日程事件", async () => {
    render(createElement(ScheduleView, {
      loading: false,
      snapshot: {
        ...snapshot,
        deadlines: [{
          id: "legacy-deadline",
          title: "孤儿截止项",
          dueAt: "2026-08-05T09:00:00+08:00",
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: "月历" })).toBeTruthy());
    expect(screen.queryByText("孤儿截止项")).toBeNull();
    // 「截止」已随死种类移除，筛选只剩四个来源。
    expect(screen.queryByRole("button", { name: "截止" })).toBeNull();
    expect(screen.getByRole("button", { name: "课程" })).toBeTruthy();
    expect(screen.getByRole("button", { name: "考试" })).toBeTruthy();
    expect(screen.getByRole("button", { name: "作业" })).toBeTruthy();
    expect(screen.getByRole("button", { name: "我的日程" })).toBeTruthy();
  });
