...
describe("practice availability", () => {
  it("keeps a source failure visible with a working retry instead of an empty-success state", async () => {
    ...
    const capabilities: PluginCapabilityClient = {
      read: async <T,>(capability: PluginCapability) => capability === "practice.records@1"
        ? [{ capability, providerId: "provider", accountId: "fixture-account", state: "unavailable", updatedAt: "2026-09-04T00:00:00Z", data: null, message: "素拓服务暂时不可用" }] as CapabilityRecord<T>[]
        : []
    };
    render(createElement(AcademicView, { capabilities, snapshot: null, loading: false, onRefresh: async () => { retryCount += 1; } }));
    fireEvent.click(screen.getByRole("button", { name: "素拓" }));
    expect(await screen.findByText("素拓服务暂时不可用")).toBeTruthy();
    expect(screen.queryByText("当前没有可用素拓记录。")).toBeNull();
    fireEvent.click(screen.getByRole("button", { name: "重新同步" }));
    await waitFor(() => expect(retryCount).toBe(1));
  });

  it("explains internal capability errors without exposing engine wording", async () => {
    const capabilities: PluginCapabilityClient = {
      read: async (capability) => {
        if (capability === "practice.records@1") throw new Error("插件请求的能力当前没有可用绑定。");
        return [];
      }
    };
    render(createElement(AcademicView, { capabilities, snapshot: null, loading: false, onRefresh: async () => undefined }));
    fireEvent.click(screen.getByRole("button", { name: "素拓" }));
    expect(await screen.findByText(/请在设置中连接统一身份认证/)).toBeTruthy();
    expect(screen.queryByText("插件请求的能力当前没有可用绑定。")).toBeNull();
  });
});
