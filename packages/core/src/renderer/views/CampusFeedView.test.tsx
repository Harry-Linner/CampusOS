...
const bridge = (): NonNullable<PluginComponentProps["campusFeed"]> => ({
  ...
  savePreferences: vi.fn(async () => ({ sources: [], items: [], notificationSettings: { keywords: [] }, lastRefresh: {} })),
  getItemDetail: vi.fn(async () => ({ itemId: "x", title: "x", url: "https://x", text: "正文", attachments: [], fetchedAt: "2026-09-05T00:00:00Z", contentHash: "detail" })),
...
describe("campus-feed view", () => {
  it("renders the settings tab with the AI connection form", async () => {
    const { getByRole, findByRole, findByText } = render(createElement(CampusFeedView, { ...baseProps, campusFeed: bridge() }));
  ...
  it("shows a real load failure and retries getSnapshot", async () => {
    const feed = bridge();
    (feed.getSnapshot as ReturnType<typeof vi.fn>)
      .mockRejectedValueOnce(new Error("偏好损坏"))
      .mockResolvedValueOnce({ sources: [], items: [], notificationSettings: { keywords: [] }, lastRefresh: {}, preferences: { profile: { identity: null, college: null, interests: [] }, onboarding: "skipped" } });
    const view = render(createElement(CampusFeedView, { ...baseProps, campusFeed: feed }));
    expect(await view.findByText("资讯读取失败")).toBeTruthy();
    fireEvent.click(view.getByRole("button", { name: "重试读取" }));
    expect(await view.findByText("还没有订阅信息源")).toBeTruthy();
    expect(feed.getSnapshot).toHaveBeenCalledTimes(2);
  });
});

describe("campus-feed subscription presentation", () => {
  it("discovers only unsubscribed sources and reacts to subscription changes regardless of switches", () => {
    const source = (id: string): FeedSourceDescriptor => ({ id, name: `官网${id} · 通知`, category: "general", tags: [], baseUrl: "https://example.edu", listUrl: "https://example.edu/list", intervalMinutes: 60, enabled: false, notificationEnabled: false });
    const a = source("a"), b = source("b");
    const snapshot: CampusFeedSnapshot = { sources: [a], catalog: [a, b], items: [], lastRefresh: {}, notificationSettings: { keywords: [] } };
    const props = { snapshot, feed: bridge(), onPreferences: vi.fn() };
    const view = render(createElement(CampusFeedSubscriptions, props));
    fireEvent.click(view.getByRole("button", { name: "发现来源" }));
    expect(view.queryByRole("heading", { name: a.name })).toBeNull();
    expect(view.getByRole("heading", { name: b.name })).toBeTruthy();
    view.rerender(createElement(CampusFeedSubscriptions, { ...props, snapshot: { ...snapshot, sources: [a, b] } }));
    expect(view.queryByRole("heading", { name: b.name })).toBeNull();
    expect(view.getByText("暂无新的信息源")).toBeTruthy();
    view.rerender(createElement(CampusFeedSubscriptions, { ...props, snapshot: { ...snapshot, sources: [b] } }));
    expect(view.getByRole("heading", { name: a.name })).toBeTruthy();
  });

  it("shows website and information type without audit or refresh diagnostics", async () => {
    const source: FeedSourceDescriptor = { id: "s-a", name: "本科生院 · 通识教育", category: "general", tags: ["教务"], baseUrl: "https://a", listUrl: "https://a/list", intervalMinutes: 60, enabled: true, verification: { status: "verified", checkedAt: null, note: "已核对正文" } };
    const view = render(createElement(CampusFeedSubscriptions, {
      snapshot: {
        sources: [source],
        items: [],
        notificationSettings: { keywords: [] },
        lastRefresh: {},
        health: { "s-a": { status: "ok", attemptedAt: "2026-09-05T00:00:00Z", succeededAt: "2026-09-05T00:00:00Z", itemCount: 1, message: "第 2 页抓取失败，已保留前 1 页结果。" } }
      },
      feed: bridge(),
      onPreferences: vi.fn()
    }));
    expect(view.queryByText(/最近成功抓取|第 2 页抓取失败|已核对/)).toBeNull();
    expect(view.getByRole("heading", { name: "本科生院 · 通识教育" })).toBeTruthy();
  });
...
describe("campus-feed view (source/tag/unread filters)", () => {
  ...
  const makeBridge = (sources: FeedSourceDescriptor[], items: FeedItemRecord[], overrides: { markRead?: ReturnType<typeof vi.fn> } = {}): NonNullable<PluginComponentProps["campusFeed"]> => ({
    ...bridge(),
    getItems: vi.fn(async (ids: string[]) => items.filter(item => ids.includes(item.id))),
    getSnapshot: vi.fn(async () => ({ sources, items, notificationSettings: { keywords: [] }, lastRefresh: {} })),
  ...
  it("excludes paused-source history from unread counts and bulk read", async () => {
    const markRead = vi.fn(async () => undefined);
    const paused = { ...sourceB, enabled: false };
    const feed = makeBridge([sourceA, paused], [
      item("1", "s-a", "new", "可见未读"),
      item("2", "s-b", "new", "暂停源未读")
    ], { markRead });
    const view = render(createElement(CampusFeedView, { ...baseProps, campusFeed: feed }));
    expect(await view.findByText("可见未读")).toBeTruthy();
    expect(view.queryByText("暂停源未读")).toBeNull();
    fireEvent.click(view.getAllByRole("button", { name: "全部已读" })[0]);
    expect(markRead).toHaveBeenCalledWith(["1"]);
  });
  ...
  it("locates and marks read the feed item named by a navigation target", async () => {
    ...
    expect(await findByText("已定位历史资讯")).toBeTruthy();
  ...
  it("loads an archived search hit outside the snapshot under its source and restores normal browsing", async () => {
    const feed = makeBridge([sourceA], [item("recent", "s-a", "read", "最近资讯")]);
    feed.getItems = vi.fn(async () => [item("old", "s-a", "read", "历史命中")]);
    Element.prototype.scrollIntoView = vi.fn();
    const view = render(createElement(CampusFeedView, { ...baseProps, campusFeed: feed, navigationTarget: { requestId: "old-search", viewId: "campus-feed", entityId: "old" } }));
    expect(await view.findByText("历史命中")).toBeTruthy();
    expect(view.queryByText("最近资讯")).toBeNull();
    expect(view.getByRole("button", { name: /学工门户/, expanded: true })).toBeTruthy();
    expect(feed.getItemDetail).not.toHaveBeenCalled();
    expect(view.queryByRole("button", { name: "正文" })).toBeNull();
    fireEvent.click(view.getByRole("button", { name: "返回每源最近 50 条" }));
    expect(await view.findByText("最近资讯")).toBeTruthy();
    expect(view.queryByText("历史命中")).toBeNull();
  });
...
describe("campus-feed view (第2层 三视图与分节折叠)", () => {
  ...
  it("lets the user remove one AI candidate before confirming the rest", async () => {
    const feed = makeBridge([sourceA], [item("1", "s-a", "通知")]);
    (feed.extractScheduleCandidates as ReturnType<typeof vi.fn>).mockResolvedValue([
      { itemId: "1", title: "本科截止", startAt: "2026-09-20T17:00:00+08:00", endAt: null, location: null, note: null, evidence: "本科生请于2026年9月20日17:00提交", type: "deadline" },
      { itemId: "1", title: "研究生截止", startAt: "2026-09-21T17:00:00+08:00", endAt: null, location: null, note: null, evidence: "研究生请于2026年9月21日17:00提交", type: "deadline" }
    ]);
    const view = render(createElement(CampusFeedView, { ...baseProps, campusFeed: feed }));
    await view.findByText("通知");
    fireEvent.click(view.getByRole("button", { name: "转为日程" }));
    expect(await view.findByText("本科截止")).toBeTruthy();
    fireEvent.click(view.getByRole("button", { name: "移除 本科截止" }));
    expect(view.queryByText("本科截止")).toBeNull();
    expect(view.getByText("研究生截止")).toBeTruthy();
  });
  it("does not run feed keyboard shortcuts from controls or the AI review dialog", async () => {
    const feed = makeBridge([sourceA], [item("1", "s-a", "通知")]);
    (feed.extractScheduleCandidates as ReturnType<typeof vi.fn>).mockResolvedValue([
      { itemId: "1", title: "截止", startAt: "2026-09-20T17:00:00+08:00", endAt: null, location: null, note: null, evidence: "请于2026年9月20日17:00提交", type: "deadline" }
    ]);
    const view = render(createElement(CampusFeedView, { ...baseProps, campusFeed: feed }));
    await view.findByText("通知");
    fireEvent.keyDown(view.getByRole("button", { name: "阅读原文" }), { key: "Enter" });
    expect(feed.openExternal).not.toHaveBeenCalled();
    fireEvent.click(view.getByRole("button", { name: "转为日程" }));
