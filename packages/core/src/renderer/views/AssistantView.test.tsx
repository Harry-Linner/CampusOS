...
describe("AssistantView", () => {
  it("parses automatically without exposing parsing modes", async () => {
    const parseMessage = vi.fn(async () => extraction);
    const assistant = createAssistantBridge({ parseMessage });
    render(createElement(AssistantView, { ...baseProps, assistant }));
    await waitFor(() => expect(assistant.loadSettings).toHaveBeenCalled());
    expect(screen.queryByLabelText("解析模式")).toBeNull();
    ...
    await waitFor(() => expect(parseMessage).toHaveBeenCalledWith(expect.objectContaining({ mode: "auto" })));
  });

  it("discovers models from the unsaved connection profile and lets the user select one", async () => {
    const assistant = createAssistantBridge();
    render(createElement(AssistantView, { ...baseProps, assistant }));
    await waitFor(() => expect(assistant.loadSettings).toHaveBeenCalled());
    fireEvent.click(screen.getByRole("button", { name: "设置" }));
    fireEvent.click(screen.getByRole("button", { name: "发现模型" }));
    await waitFor(() => expect(assistant.discoverModels).toHaveBeenCalledWith(expect.objectContaining({ provider: "openai", baseUrl: settings.baseUrl })));
    fireEvent.change(screen.getByLabelText("可用模型"), { target: { value: "discovered-model" } });
    expect((screen.getByLabelText("模型") as HTMLInputElement).value).toBe("discovered-model");
  });

  it("loads a ready desktop-pet result without paying for a second parse", async () => {
    const assistant = createAssistantBridge();
    const dismiss = vi.fn(async () => undefined);
    const desktopPet: NonNullable<PluginComponentProps["desktopPet"]> = {
      show: vi.fn(async () => undefined),
      getState: vi.fn(async () => ({ settings: { enabled: true, scale: 1, alwaysOnTop: true, clickThrough: false, shortcut: "Ctrl+Shift+Space", appearance: "auto" as const }, jobs: [], shortcutRegistered: true })),
      saveSettings: vi.fn(async () => ({ settings: { enabled: false, scale: 1, alwaysOnTop: true, clickThrough: false, shortcut: "Ctrl+Shift+Space", appearance: "auto" as const }, jobs: [], shortcutRegistered: false })),
      getJob: vi.fn(async () => ({ id: "job-1", createdAt: "2026-08-05T00:00:00.000Z", status: "ready" as const, kind: "text" as const, label: "文字消息", message: "ready", result: extraction, settings })),
      dismiss,
      subscribe: vi.fn(() => () => undefined)
    };
    render(createElement(AssistantView, { ...baseProps, assistant, desktopPet, navigationTarget: { requestId: "nav-1", viewId: "ai-assistant", entityId: "pet:job-1" } }));
    ...
    fireEvent.click(screen.getByRole("button", { name: "关闭桌面助手" }));
    expect(desktopPet.saveSettings).toHaveBeenCalledWith({ enabled: false });
    expect(screen.queryByLabelText("解析模式")).toBeNull();
    expect((screen.getByLabelText("粘贴消息") as HTMLTextAreaElement).value).toBe(extraction.sourceText);
    fireEvent.click(screen.getByRole("button", { name: "清除桌宠记录" }));
  ...
  it("renders multiple candidates and unresolved questions", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "交给 AI 解析" }));
    await waitFor(() => expect(screen.getByText("2 个待确认事项")).toBeTruthy());
  ...
  it("switches to the read-only data-query mode and renders the answer with evidence sources", async () => {
    ...
    await waitFor(() => expect(screen.getByText("只读本地数据")).toBeTruthy());
