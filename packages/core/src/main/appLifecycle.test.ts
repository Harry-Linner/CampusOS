...
describe("app lifecycle", () => {
  it("offers one pet menu action and changes its label with the enabled state", async () => {
    await createStorageRoot();
    const lifecycle = await import("./appLifecycle");
    let enabled = false;
    const toggle = vi.fn(async () => { enabled = !enabled; });
    await lifecycle.createCampusTray({ desktopPet: { isVisible: () => enabled, toggle } });
    expect(electronState.menuTemplate.filter(item => item.label?.includes("桌宠")).map(item => item.label)).toEqual(["显示桌宠"]);
    electronState.menuTemplate.find(item => item.label === "显示桌宠")?.click?.();
    await vi.waitFor(() => expect(electronState.menuTemplate.filter(item => item.label?.includes("桌宠")).map(item => item.label)).toEqual(["关闭桌宠"]));
    electronState.menuTemplate.find(item => item.label === "关闭桌宠")?.click?.();
    await vi.waitFor(() => expect(electronState.menuTemplate.filter(item => item.label?.includes("桌宠")).map(item => item.label)).toEqual(["显示桌宠"]));
  });
