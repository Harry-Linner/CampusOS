  ...
  try {
    ...
    try {
      ...
      if (controller.signal.aborted) {
        throw new AiProviderAdapterError("network-error", callerSignal?.aborted ? "AI 请求已取消。" : "AI 服务响应超时，请稍后重试。", { cause });
      }
      if (!response.ok) {
    ...
    if (cause instanceof AiProviderAdapterError) throw cause;
    if (controller.signal.aborted) {
      throw new AiProviderAdapterError(
        "network-error",
        callerSignal?.aborted ? "AI 请求已取消。" : "AI 服务响应超时，请稍后重试。",
        { cause }
      );
    }
...
const dataUrl = (image: NonNullable<StructuredGenerationInput["images"]>[number]): string =>
  `data:${image.mime};base64,${image.base64}`;

const responsesInput = (input: StructuredGenerationInput): string | unknown[] => input.images?.length
  ? [{ role: "user", content: [
      { type: "input_text", text: JSON.stringify(input.input) },
      ...input.images.map((image) => ({ type: "input_image", image_url: dataUrl(image), detail: "auto" }))
    ] }]
  : JSON.stringify(input.input);

const chatInput = (input: StructuredGenerationInput): string | unknown[] => input.images?.length
  ? [
      { type: "text", text: JSON.stringify(input.input) },
      ...input.images.map((image) => ({ type: "image_url", image_url: { url: dataUrl(image) } }))
    ]
  : JSON.stringify(input.input);

const anthropicInput = (input: StructuredGenerationInput): string | unknown[] => input.images?.length
  ? [
      ...input.images.map((image) => ({ type: "image", source: { type: "base64", media_type: image.mime, data: image.base64 } })),
      { type: "text", text: JSON.stringify(input.input) }
    ]
  : JSON.stringify(input.input);
        ...
        const payload = await requestJson(fetchFn, joinUrl(profile.baseUrl, "chat/completions"), {
          ...
          body: JSON.stringify({
            ...
            messages: [
              ...
              { role: "user", content: chatInput(input) }
            ],
      ...
      generateStructured: async (input) => {
        const payload = await requestJson(fetchFn, joinUrl(profile.baseUrl, "messages"), {
          ...
          body: JSON.stringify({
            ...
            messages: [{ role: "user", content: anthropicInput(input) }],
        ...
        const payload = await requestJson(fetchFn, joinUrl(profile.baseUrl, `models/${encodeURIComponent(profile.model)}:generateContent`), {
          ...
          body: JSON.stringify({
            contents: [{ role: "user", parts: [
              ...
              ...(input.images ?? []).map((image) => ({ inlineData: { mimeType: image.mime, data: image.base64 } }))
            ] }],
            generationConfig: {
              temperature: 0,
