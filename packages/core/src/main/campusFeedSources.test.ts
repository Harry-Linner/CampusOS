...
describe("campusFeedSources", () => {
  it("defines default sources: enabled ones carry selectors or an adapter, candidates are disabled", () => {
    ...
    for (const source of enabled) {
      expect(source.selectors?.container || ["rss", "zju-hospital"].includes(source.adapterId ?? "")).toBeTruthy();
  ...
  it("drops list entries outside the source allowlist", async () => {
    const html = `
    <ul class="news_list">
      <li class="news"><span class="news_title"><a href="/2026/0101/c1a1/page.htm">校内通知</a></span><span class="news_meta">2026-01-01</span></li>
      <li class="news"><span class="news_title"><a href="https://example.com/tracker">站外链接</a></span><span class="news_meta">2026-01-01</span></li>
    </ul>`;
    const { items } = await fetchSourceList(descriptor("xgb-pingjiang"), { fetchFn: mockFetch(html) });
    expect(items.map((item) => item.title)).toEqual(["校内通知"]);
  });
  ...
  it("classifies login and campus-network HTTP responses as restricted", async () => {
    const restricted = vi.fn(async () => new Response("forbidden", { status: 403 })) as unknown as typeof fetch;
    await expect(fetchSourceList(descriptor("xgb-pingjiang"), { fetchFn: restricted }))
      .rejects.toMatchObject({ code: "restricted" });
  });

  it("rejects HTTPS-to-HTTP links even on an allowed hostname", () => {
    expect(isFeedSourceUrl(descriptor("intl-rss"), "http://www.intl.zju.edu.cn/news/unsafe")).toBe(false);
  });

  it("parses a Drupal RSS feed via the rss adapter", async () => {
    const rss = `
    <rss version="2.0"><channel><title>Intl Campus</title>
      <item><title>Opening of the new library wing</title><link>https://lib.intl.zju.edu.cn/news/opening</link><pubDate>Wed, 20 Aug 2026 08:00:00 +0000</pubDate><description>Grand opening ceremony.</description></item>
      <item><title>No-link item skipped</title></item>
    </channel></rss>`;
    const { items } = await fetchSourceList(descriptor("libintl-rss"), { fetchFn: mockFetch(rss) });
    expect(items).toHaveLength(1);
    expect(items[0]).toMatchObject({
      sourceId: "libintl-rss",
      title: "Opening of the new library wing",
      url: "https://lib.intl.zju.edu.cn/news/opening",
      summary: "Grand opening ceremony."
    });
    expect(items[0].publishedAt).not.toBeNull();
  });

  it("retains complete list titles and summaries for historical keyword search", async () => {
    const title = `${"标题".repeat(180)}末尾标题关键词`;
    const summary = `${"摘要".repeat(180)}末尾摘要关键词`;
    const rss = `<rss><channel><item><title>${title}</title><link>https://lib.intl.zju.edu.cn/news/archive</link><description>${summary}</description></item></channel></rss>`;
    const result = await fetchSourceList(descriptor("libintl-rss"), { fetchFn: mockFetch(rss) });
    expect(result.items[0]).toMatchObject({ title, summary });
  });
  ...
  it("decodes GBK-encoded list pages when encoding is set", async () => {
    ...
    const gbkHtml = new Uint8Array([
      ...ascii('<div class="right"><ul class="list"><li class="list-item"><a href="/redir.php?object_id=1"><div class="info"><span class="title">'),
      0xd6, 0xd0, 0xce, 0xc4,
      ...ascii('</span></div><div class="date"><span class="y">2026-01</span><span class="d">01</span></div></a></li></ul></div>')
    ]);
  ...
  it("cancels a chunked response as soon as its body exceeds the byte limit", async () => {
    let pulls = 0;
    let cancelled = false;
    ...
    await expect(fetchSourceList(descriptor("xgb-pingjiang"), { fetchFn: fetcher }))
      .rejects.toMatchObject({ code: "layout-changed" });
    expect(cancelled).toBe(true);
    // WHATWG streams may prefetch one chunk, but the source must be cancelled
    // before all six chunks are consumed.
    expect(pulls).toBeLessThanOrEqual(5);
  });

  it("fetches additional sudic pages when maxPages is set", async () => {
    const page1 = `<li class="right-list-item"><a href="/2026/0101/c1a1/page.htm"><p>一</p><div class="time"><span class="y">2026-01-01</span></div></a></li><a href="list2.htm">下一页</a>`;
    const page2 = `<li class="right-list-item"><a href="/2026/0102/c1a2/page.htm"><p>二</p><div class="time"><span class="y">2026-01-02</span></div></a></li>`;
  ...
  it("keeps first-page results and reports a partial warning when a later page fails", async () => {
    const page1 = `<li class="right-list-item"><a href="/2026/0101/c1a1/page.htm"><p>一</p><div class="time"><span class="y">2026-01-01</span></div></a></li><a href="list2.htm">下一页</a>`;
    const fetcher = vi.fn(async (input: RequestInfo | URL) =>
      String(input).endsWith("list.htm")
        ? new Response(page1, { status: 200 })
        : new Response("temporary failure", { status: 503 })
    ) as unknown as typeof fetch;
    const outcome = await fetchSourceList(descriptor("bksy-tzgg"), { fetchFn: fetcher });
    expect(outcome.items.map((item) => item.title)).toEqual(["一"]);
    expect(outcome.partialWarning).toMatch(/第 2 页抓取失败.*保留前 1 页/);
  });

  it("does not guess a numbered page when the website exposes no next link", async () => {
    const page1 = `<li class="right-list-item"><a href="/2026/0101/c1a1/page.htm"><p>一</p><div class="time"><span class="y">2026-01-01</span></div></a></li>`;
    const fetcher = vi.fn(async () => new Response(page1, { status: 200 })) as unknown as typeof fetch;
    const { items } = await fetchSourceList(descriptor("bksy-tzgg"), { fetchFn: fetcher });
    expect(items.map((item) => item.title)).toEqual(["一"]);
