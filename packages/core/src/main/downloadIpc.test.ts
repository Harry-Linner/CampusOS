...
describe("download IPC", () => {
  ...
  it("verifies completed files and clears terminal history through trusted handlers", async () => {
    await expect(invoke("campusos:downloads:verify", readyTask.id)).resolves.toMatchObject({ status: "verified" });
