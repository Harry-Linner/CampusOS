...
const invoke = async <T>(channel: string, ...args: unknown[]): Promise<T> => {
  ...
  return await handler({ senderFrame: frame, sender: { mainFrame: frame } }, ...args) as T;
};
...
describe("desk calendar host", () => {
  it("rejects calendar access from a foreign renderer or child frame", async () => {
    const handler = electronState.handlers.get("campusos:desk-calendar:data")!;
    const foreign = { url: "https://example.invalid/desk-calendar.html" };
    expect(() => handler({ senderFrame: foreign, sender: { mainFrame: foreign } })).toThrow(/untrusted origin/);
    const trusted = { url: new URL("../renderer/index.html", import.meta.url).href };
    expect(() => handler({ senderFrame: trusted, sender: { mainFrame: { ...trusted } } })).toThrow(/untrusted frame/);
  });
