...
describe("connected source catalog and capability status", () => {
  it("only lists implemented connectors and prunes legacy planned rows", () => {
    const snapshot = createLiveWorkspaceSnapshot({ generatedAt: "2026-09-04T00:00:00Z", accountId: "account" });
    expect(snapshot.sourceStates.map((row) => row.sourceId)).toEqual(["academic-affairs", "learning-platform"]);
    const legacy = { ...snapshot, sourceStates: [...snapshot.sourceStates, { ...snapshot.sourceStates[0], sourceId: "cs-college" as const, status: "planned" as const }] };
    expect(pruneUnsupportedWorkspaceSources(legacy).summary.totalSources).toBe(2);
  });

  it("replaces counts from materials, grades and practice without counting a second account or projections twice", () => {
    const snapshot = createLiveWorkspaceSnapshot({ generatedAt: "2026-09-04T00:00:00Z", accountId: "account" });
    const base = { accountId: "account", updatedAt: snapshot.generatedAt, state: "live" as const };
    const records: CapabilityRecord<unknown>[] = [
      { ...base, capability: "learning.materials@1", providerId: "org.campusos.zju-learning", data: { materials: [{ sourceId: "m1" }] } },
      { ...base, capability: "academic.grades@1", providerId: "academic", data: { grades: [{ sourceId: "g1" }, { sourceId: "g2" }] } },
      { ...base, capability: "practice.records@1", providerId: "academic", state: "cache", data: { records: [] }, message: "素拓暂不可用，保留上次记录" },
      { ...base, accountId: "other-account", capability: "academic.grades@1", providerId: "academic", data: { grades: Array(20).fill({}) } }
    ];
    const merged = mergeWorkspaceSourceStates(snapshot, records, "academic", "account");
    expect(merged.sourceStates.find((row) => row.sourceId === "learning-platform")).toMatchObject({ status: "ready", itemCount: 1 });
    expect(merged.sourceStates.find((row) => row.sourceId === "academic-affairs")).toMatchObject({ status: "partial", itemCount: 2, itemCountsByCapability: { "academic.grades@1": 2, "practice.records@1": 0 } });
    expect(merged.sourceStates[0].summary).toContain("素拓暂不可用");
    expect(mergeWorkspaceSourceStates(merged, records, "academic", "account")).toEqual(merged);
  });
});
