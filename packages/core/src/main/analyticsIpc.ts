...
export const registerAnalyticsHandlers = (): void => {
  registerTrustedIpcHandler("campusos:analytics:load", async () => { return readAnalyticsRecord(); });
  registerTrustedIpcHandler("campusos:analytics:set-consent", async (consent: unknown) => { if (typeof consent !== "boolean") throw new Error("Analytics consent is invalid."); const settings = await readSettings(); const next = { ...settings, consent, savedAt: new Date().toISOString() }; await saveSettings(next); return toRecord(next); });
  registerTrustedIpcHandler("campusos:analytics:track", async (name: unknown) => { if (!isEvent(name)) throw new Error("Unsupported analytics event."); try { return { accepted: await trackEvent(name) }; } catch { return { accepted: false }; } });
};
