...
let desktopPetActions: { isVisible: () => boolean; toggle: () => void | Promise<void> } | null = null;
...
export const showCampusMainWindow = (): boolean => {
...
export const navigateCampusMainWindow = (
  target: Omit<AppNavigationRequest, "requestId"> & { requestId?: string }
): boolean => {
  if (!showCampusMainWindow() || !mainWindow || mainWindow.isDestroyed()) return false;
  mainWindow.webContents.send("campusos:navigation:request", {
    ...target,
    requestId: target.requestId ?? randomUUID()
...
export const campusIconPath = (): string => app.isPackaged
  ? join(process.resourcesPath, "icon.ico")
  : developmentTrayIconPath();

export const rebuildTrayMenu = async (): Promise<void> => {
  if (!tray) return;
  const template: MenuItemConstructorOptions[] = [
    { label: "打开 CampusOS", click: showCampusMainWindow },
    ...(desktopPetActions ? [{
      label: desktopPetActions.isVisible() ? "关闭桌宠" : "显示桌宠",
      click: () => { void Promise.resolve(desktopPetActions?.toggle()).then(rebuildTrayMenu).catch(() => undefined); }
    }] : []),
    {
      label: "桌面日历",
      click: () => {
...
export const createCampusTray = async (options?: { desktopPet?: { isVisible: () => boolean; toggle: () => void | Promise<void> } }): Promise<void> => {
  desktopPetActions = options?.desktopPet ?? null;
  if (tray) return;
  tray = new Tray(campusIconPath());
...
export const registerAppLifecycleHandlers = (): void => {
  registerTrustedIpcHandler("campusos:lifecycle:load", async () => loadSettings());
  registerTrustedIpcHandler("campusos:lifecycle:save", async (patch: AppLifecycleSettingsPatch) =>
