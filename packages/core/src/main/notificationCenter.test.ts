...
const electronState = vi.hoisted(() => ({
  ...
  userDataPath: "",
  notifications: [] as Array<{ options: Record<string, unknown>; handlers: Map<string, () => void>; show: ReturnType<typeof vi.fn> }>,
  navigate: vi.fn()
}));
...
vi.mock("./appLifecycle", () => ({
  ...
  campusIconPath: () => "campus-icon.ico"
}));
...
vi.mock("electron", () => ({
  ...
  Notification: class {
    ...
    constructor(options: Record<string, unknown> = {}) {
      ...
      electronState.notifications.push(this as unknown as typeof electronState.notifications[number]);
    }
...
describe("Notification IPC", () => {
  ...
  it("brands every desktop toast with the app icon", async () => {
    await addNotification({ id: "icon-target", kind: "system", title: "带图标", body: "正文" });
    expect(electronState.notifications).toHaveLength(1);
    expect(electronState.notifications[0].options).toMatchObject({
      title: "带图标",
      body: "正文",
      icon: "campus-icon.ico"
    });

    await showTransientNotification({ title: "下载完成", body: "文件已保存" });
    expect(electronState.notifications).toHaveLength(2);
    expect(electronState.notifications[1].options).toMatchObject({
      title: "下载完成",
      body: "文件已保存",
      icon: "campus-icon.ico"
    });
