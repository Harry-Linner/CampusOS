...
export const registerAcademicCalendarHandlers = (options?: { onSaved?: () => void | Promise<void> }): void => {
  registerTrustedIpcHandler("campusos:academic-calendar:settings:load", async () =>
  ...
  registerTrustedIpcHandler(
    ...
    async (input: unknown) => {
      const candidate = typeof input === "object" && input !== null
        ...
        : {};
      const result = await saveAcademicCalendarSettings(candidate);
