...
export class DownloadEngine {
  ...
  async verify(id: string): Promise<CampusDownloadVerification> {
    const item = this.items.get(id);
    if (!item || item.status !== "ready") throw new Error("下载完成后才能校验文件。");
    const expectedBytes = item.totalBytes > 0 ? item.totalBytes : item.expectedBytes ?? null;
    try {
      const actualBytes = (await stat(item.targetPath)).size;
      return {
        status: expectedBytes !== null && actualBytes !== expectedBytes ? "size-mismatch" : "verified",
        actualBytes,
        expectedBytes
      };
    } catch (error) {
      if (typeof error === "object" && error !== null && "code" in error && error.code === "ENOENT") {
        return { status: "missing", actualBytes: null, expectedBytes };
      }
      throw error;
    }
  }
