...
import { readAcademicCredentialRecord, readAcademicCredentialSecret } from "./academicCredentialStore";
...
import {
  ...
  mergeWorkspaceSourceStates,
  pruneUnsupportedWorkspaceSources,
...
const buildGeneratedRecord = async (
  ...
  if (initialCredential.verificationState === "verified" && !useE2eFixtureSources()) {
    await readAcademicCredentialSecret();
  }
  const credentialScope = (record: typeof initialCredential): string => JSON.stringify([record.verificationState, record.authenticatedProfile?.studentId, record.program]);
  const scopeKey = credentialScope(initialCredential);
  const pluginRuntime = await getOfficialPluginRuntimeService().loadInternal();
  const refreshResults = await pluginRefreshCoordinator.runAll({ background, scopeKey });
  ...
  if (credentialScope(academicCredential) !== scopeKey) throw new Error("账号已改变，请重新同步当前账号。");
  ...
  const materialSnapshot = mergeLearningMaterialsIntoWorkspace(
    eventSnapshot,
    findLearningMaterialsRecord(
  ...
  const sourceRecords = (await Promise.all([
    "academic.timetable@1", "academic.exams@1", "academic.grades@1", "practice.records@1", "learning.assignments@1"
  ].map((capability) => getOfficialCapabilityRepository().read<unknown>(capability as `${string}@${number}`)))).flat();
  const mergedSnapshot = mergeWorkspaceSourceStates(
    materialSnapshot,
    [...sourceRecords, ...learningMaterialRecords, ...findCalendarEventRecords(eventRecords, activeEventProviderIds, verifiedAcademicAccountId)],
    getAcademicConnectorSourceId(academicCredential.program ?? "undergraduate"),
    verifiedAcademicAccountId
  );
...
let pendingWorkspaceSync: { background: boolean; operation: Promise<CampusWorkspaceRecord> } | null = null;
export const syncCampusWorkspace = async (
  options: { notifyGradeChanges?: boolean; background?: boolean } = {}
): Promise<CampusWorkspaceRecord> => {
  const current = pendingWorkspaceSync;
  if (current) {
    // Manual refresh bypasses a background throttle once its in-flight work finishes.
    if (!options.background && current.background) {
      await current.operation.catch(() => undefined);
      return syncCampusWorkspace(options);
    }
    return current.operation;
  }
  const operation = buildGeneratedRecord("synced", options.notifyGradeChanges !== false, options.background === true)
    .finally(() => { if (pendingWorkspaceSync?.operation === operation) pendingWorkspaceSync = null; });
  pendingWorkspaceSync = { background: options.background === true, operation };
  return operation;
};
...
export const registerCampusWorkspaceHandlers = (options?: { onSynced?: () => void | Promise<void> }): void => {
  registerTrustedIpcHandler("campusos:workspace:hydrate", async () =>
    hydrateCampusWorkspace()
  );
  registerTrustedIpcHandler("campusos:workspace:sync", async () => {
    const result = await syncCampusWorkspace();
    if (options?.onSynced) await options.onSynced();
    return result;
  });
};
