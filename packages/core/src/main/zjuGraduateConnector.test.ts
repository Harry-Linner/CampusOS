...
describe("zju graduate connector", () => {
  it("uses four request terms and treats response terms 15/16 as full-semester flags", () => {
    const queries = createGraduateTimetableQueries(new Date("2026-09-04T00:00:00Z"));
    expect(queries.filter((query) => query.academicYearStart === 2026).map((query) => query.term)).toEqual([13, 14, 11, 12]);
    for (const responseTerm of [15, 16]) {
