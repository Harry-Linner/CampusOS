...
const service = {
  ...
  savePreferences: vi.fn(async () => snapshot),
  getItemDetail: vi.fn(async () => ({ itemId: "i1", text: "正文" })),
...
describe("campusFeedIpc", () => {
  it("registers the formal feed channels including onboarding and article details", () => {
    process.env.ELECTRON_RENDERER_URL = "http://localhost:5173/";
    registerCampusFeedHandlers(service);
    expect([...electronState.handlers.keys()]).toEqual([
      "campusos:campus-feed:get",
      "campusos:campus-feed:search-history",
      "campusos:campus-feed:get-items",
      "campusos:campus-feed:refresh-source",
      "campusos:campus-feed:add-source",
      "campusos:campus-feed:preferences-save",
      "campusos:campus-feed:item-detail",
  ...
  it("forwards subscriptions and details through trusted channels", async () => {
    await invoke("campusos:campus-feed:add-source", "s1");
    expect(service.addSource).toHaveBeenCalledWith("s1");
    const input = { profile: { identity: null, college: null, interests: [] }, selectedSourceIds: ["s1"] };
    expect(await invoke("campusos:campus-feed:preferences-save", input)).toBe(snapshot);
    expect(service.savePreferences).toHaveBeenCalledWith(input);
    expect(await invoke("campusos:campus-feed:item-detail", "i1")).toMatchObject({ itemId: "i1" });
    await expect(invoke("campusos:campus-feed:add-source", null)).rejects.toThrow(/订阅源 ID/);
    await expect(invoke("campusos:campus-feed:item-detail", 1)).rejects.toThrow(/通知 ID/);
  });
