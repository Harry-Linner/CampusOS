...
describe("database service", () => {
  it.each([10, 14])("upgrades v%s without losing collaboration preferences or upstream notifications", async (version) => {
    const root = await mkdtemp(join(tmpdir(), "campusos-merge-migration-"));
    temporaryDirectories.push(root);
    const databasePath = join(root, "campusos.sqlite");
    const initial = createDatabaseService({ databasePath });
    initial.saveCampusFeedPreferences({ onboarding: "completed" }, "2026-09-06T00:00:00Z");
    initial.saveCampusFeedDetail("fixture", { contentHash: "original" }, "2026-09-06T00:00:00Z");
    initial.close();
    const old = new Database(databasePath);
    old.prepare("DELETE FROM schema_migrations WHERE version > ?").run(version);
    if (version === 10) {
      old.exec(`DROP TABLE campus_feed_refresh_state; DROP TABLE desktop_calendar_state;
        DROP TABLE notifications; DROP TABLE notification_storage_meta;
        DROP TABLE campus_feed_notification_settings; CREATE TABLE planner_schedules (value TEXT);`);
    } else {
      old.prepare("INSERT INTO notifications VALUES (?, ?)").run("fixture", JSON.stringify({ id: "fixture", status: "unread" }));
      old.exec("DROP TABLE campus_feed_preferences; DROP TABLE campus_feed_details;");
    }
    old.close();
    const migrated = createDatabaseService({ databasePath });
    try {
      expect(migrated.schemaVersion).toBe(15);
      if (version === 10) {
        expect(migrated.loadCampusFeedPreferences()).toEqual({ onboarding: "completed" });
        expect(migrated.loadCampusFeedDetail("fixture")).toEqual({ contentHash: "original" });
        expect(migrated.loadNotifications()).toEqual([]);
      } else {
        expect(migrated.loadNotifications()).toEqual([{ id: "fixture", status: "unread" }]);
        expect(migrated.loadCampusFeedPreferences()).toBeNull();
      }
