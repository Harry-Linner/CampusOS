...
describe("DownloadEngine", () => {
  ...
  it("sorts persisted history newest first, verifies files, and clears terminal records", async () => {
    ...
    temporaryDirectories.push(storageRoot);
    const targetPath = join(storageRoot, "ready.pdf");
    await writeFile(targetPath, "four", "utf8");
    const base = {
      url: "https://example.edu/file.pdf", title: "ready.pdf", courseName: "Course",
      sourceId: "learning-platform" as const, semester: "2026", targetPath,
      temporaryPath: `${targetPath}.part`, totalBytes: 4, downloadedBytes: 4,
      updatedAt: "2026-09-01T00:00:00.000Z"
    };
    let saved: DownloadQueueItem[] = [];
    const engine = new DownloadEngine({
      downloadRoot: storageRoot,
      persistencePath: join(storageRoot, "q.json"),
      queuePersistence: {
        load: async () => [
          { ...base, id: "old", status: "ready" as const, createdAt: "2026-08-01T00:00:00.000Z", expectedBytes: 4 },
          { ...base, id: "new", status: "failed" as const, createdAt: "2026-09-01T00:00:00.000Z" },
          { ...base, id: "active", status: "paused" as const, createdAt: "2026-07-01T00:00:00.000Z" }
        ],
        save: async (queue) => { saved = [...queue]; }
      }
    });
    await engine.loadPersisted();
    expect(engine.getSummary().map((item) => item.id)).toEqual(["new", "old", "active"]);
    await expect(engine.verify("old")).resolves.toEqual({ status: "verified", actualBytes: 4, expectedBytes: 4 });
    await expect(engine.clearHistory()).resolves.toBe(2);
    expect(engine.getSummary().map((item) => item.id)).toEqual(["active"]);
