...
import type { ZjuAuthenticationResult } from "./zjuAuthContracts";
...
export const isVerifiedAcademicCredentialPayload = (value: unknown): value is StoredAcademicCredentialPayload =>
  isStoredPayload(value) && isVerifiedPayload(value);
...
export const createAcademicCredentialService = ({
  ...
  loadSecret: async (): Promise<AcademicCredentialSecret | null> => {
    ...
    try {
      ...
      throw new AcademicCredentialServiceError("storage-error", "本地登录凭据无法解密，请在账号设置中重新连接。已保存的课表和其他数据不会被删除。", { cause: error });
    }
  }
});
