...
describe("academic timetable events", () => {
  it("reports missing period times without inventing the fifteenth period", () => {
    const missingPeriodRecord = { ...timetableRecord, data: { terms: timetableRecord.data!.terms.map((term) => ({ ...term, sessions: term.sessions.map((session) => ({ ...session, periods: [15] })) })) } };
    const result = deriveTimetableCalendarEvents([missingPeriodRecord], calendarConfig, "2026-09-04T00:00:00Z");
    expect(result.events).toEqual([]);
    expect(result.omittedItems).toBeGreaterThan(0);
    expect(result.diagnostics).toContain("第 15 节缺少已核实的上课时间，相关课程仍可在课表中查看。");
  });
  ...
  it("expands a weekly course session into concrete calendar events", () => {
    ...
    // 学季区间起始日 2026-09-11（报到注册日）不是上课日：
    // 任何课程事件都不得早于 classesBeginDate。
    expect(result.events.some((event) => event.startAt.startsWith("2026-09-11"))).toBe(false);
    expect(result.events.every((event) => event.startAt >= "2026-09-14")).toBe(true);
