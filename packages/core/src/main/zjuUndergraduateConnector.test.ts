...
describe("zju undergraduate connector", () => {
  ...
  it("reports blank and HTML timetable responses without exposing a JSON parse error", () => {
    const query = { academicYearStart: 2025, season: "1|秋" as const };
    expect(() => parseTimetableResponse(query, " \n\t")).toThrow(
      "教务网未返回课表内容，请稍后重试。"
    );
    expect(() => parseTimetableResponse(query, "<!doctype html><html>login</html>")).toThrow(
      "教务网返回了网页而非课表数据，请检查登录状态或稍后重试。"
    );
    expect(() => parseTimetableResponse(query, "not-json")).toThrow(
      "教务网课表数据格式异常，暂时无法读取。"
    );
  });
  ...
  it("preserves the new transcript but labels the cached major summary as degraded", async () => {
    let refreshJob: (() => Promise<unknown>) | undefined;
    const publish = vi.fn(async () => undefined);
    const connector = createZjuUndergraduateConnector({
      loadAcademicProfileProof: vi.fn(async () => ({
    ...
    expect(publish).toHaveBeenCalledWith(expect.objectContaining({
      ...
      data: expect.objectContaining({
        ...
        majorSummary: expect.objectContaining({ fivePointGpa: 3, state: "cache" })
      })
    }));
  });
  ...
  it.each([true, false])("keeps matching successful cache when every live term fails (has courses: %s)", async (hasCourses) => {
    const cached = {
      terms: [
        {
          academicYearStart: 2025,
    ...
    const publish = vi.fn<(publication: unknown) => Promise<void>>(async () => undefined);
    const connector = createZjuUndergraduateConnector({
    ...
    const timetable = publish.mock.calls
      ...
      .filter((publication) => publication.capability === "academic.timetable@1").at(-1);
      ...
      data: expect.objectContaining({
        terms: expect.arrayContaining([
          expect.objectContaining({
            ...
            season: "2|夏",
            state: "cache",
            sessions: cached.terms[0].sessions
          })
        ])
      })
    }));
    expect(timetable?.data?.terms).toHaveLength(12);
    expect(timetable?.message?.match(/临时网络错误/g)).toHaveLength(1);
  });
  ...
  it("retains per-term failure reasons when no timetable cache exists", async () => {
    ...
    expect(publish).toHaveBeenCalledWith(expect.objectContaining({
      ...
      data: { terms: expect.arrayContaining([expect.objectContaining({ academicYearStart: 2025, season: "1|秋", message: expect.stringContaining("service-unavailable") })]) }
    }));
  });

  it.each(["null", "   ", '{"kbList":[]}'])("protects only the matching non-empty cache from %s", async (emptyBody) => {
    let refreshJob: (() => Promise<unknown>) | undefined;
    const publish = vi.fn<(publication: unknown) => Promise<void>>(async () => undefined);
    const cachedSession = {
      sourceId: "cached-session",
    ...
    const connector = createZjuUndergraduateConnector({
      ...
      fetchTimetableTerms: async (queries) => queries.map((query) => ({
        query,
        ok: true as const,
        body: query.academicYearStart === 2024 && query.season === "1|秋"
          ? emptyBody
          : JSON.stringify({ kbList: [] })
      })),
      loadCachedTimetable: async () => cached,
    ...
    const refreshResult = await refreshJob?.() as {
      status?: string;
      message?: string;
    } | undefined;
    const timetablePublications = publish.mock.calls
      .map(([publication]) => publication as {
        capability?: string;
        data?: { terms?: unknown[] };
      })
      .filter((publication) => publication.capability === "academic.timetable@1");
    const timetable = timetablePublications.at(-1);

    expect(timetable?.data?.terms).toHaveLength(12);
    expect(timetable?.data?.terms).toEqual(expect.arrayContaining([
      expect.objectContaining({
        academicYearStart: 2024,
        season: "1|秋",
        state: "cache",
    ...
    expect(refreshResult).toEqual(expect.objectContaining({
      status: "fallback",
      message: expect.stringContaining("已保留同学期缓存")
    }));
    expect(refreshResult?.message).not.toContain("部分模块已实时刷新");
  });

  it("treats a non-empty kbList that yields no sessions as a parse failure and preserves the matching cache", async () => {
    const publish = vi.fn<(publication: unknown) => Promise<void>>(async () => undefined);
    const cachedSession = {
      sourceId: "cached-session",
      courseName: "缓存课程",
    ...
    const connector = createZjuUndergraduateConnector({
      ...
      fetchTimetableTerms: async (queries) => queries.map((query) => ({
        ...
        ok: true as const,
        body: query.academicYearStart === 2024 && query.season === "1|秋"
          ? JSON.stringify({ kbList: [{ kcb: "损坏条目" }] })
          : JSON.stringify({ kbList: [] })
      })),
      loadCachedTimetable: async () => cached,
      fetchExams: async () => ({ ok: true as const, body: JSON.stringify({ items: [] }) }),
