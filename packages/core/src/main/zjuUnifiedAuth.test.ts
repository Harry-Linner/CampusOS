...
import type { ZjuAuthHttpRequest, ZjuAuthHttpResponse, ZjuAuthTransport } from "./zjuAuthContracts";
...
describe("ZjuUnifiedAuthClient", () => {
  ...
  it("surfaces a localized network error after exhausting learning API retries", async () => {
    vi.useFakeTimers();
    try {
      const routed = createAuthenticatedUndergraduateTransport();
      let apiAttempts = 0;
      ...
      const assertion = expect(operation).rejects.toMatchObject({
        code: "network-error",
        message: "无法连接统一认证服务，请检查网络后重试。"
      });

      await vi.runAllTimersAsync();
      await assertion;
      expect(apiAttempts).toBe(6);
    } finally {
      vi.useRealTimers();
    }
  });
