...
export const registerBackupHandlers = (): void => {
  registerTrustedIpcHandler("campusos:backup:export", async () => {
  ...
  registerTrustedIpcHandler("campusos:backup:preview", async () => {
  ...
  registerTrustedIpcHandler("campusos:backup:restore", async (mode: BackupRestoreMode) => {
