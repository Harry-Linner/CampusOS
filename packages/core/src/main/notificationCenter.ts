...
import { campusIconPath, getAppLifecycleSettings, navigateCampusMainWindow } from "./appLifecycle";
...
/** Suppress persistence-triggered desktop toasts while switching account profiles. */
export const suppressNotifications = (): void => {
...
export const showTransientNotification = async (input: TransientNotificationInput): Promise<void> => {
  ...
  // Windows toast 的默认 logo 取自 AUMI 关联的快捷方式；显式传 icon 才会用应用图标，
  // 否则开发态或身份未注册时会显示 Electron 默认图标。
  const notification = new Notification({ title: input.title, body: input.body, icon: campusIconPath() });
...
export const addNotification = async (input: AddNotificationInput): Promise<NotificationRecord> => {
  ...
  persist(records);
  if (!notificationsSuppressed && input.showDesktop !== false) {
    const target = targetToNavigation(input.desktopActionTarget ?? record.actionTarget);
    const lifecycle = await getAppLifecycleSettings();
    if (!notificationsSuppressed && lifecycle.notificationEnabled && Notification.isSupported()) {
      const notification = new Notification({
        title: input.desktopTitle ?? record.title,
        body: input.desktopBody ?? record.body,
        icon: campusIconPath()
      });
...
export const registerNotificationHandlers = (): void => {
  registerTrustedIpcHandler("campusos:notifications:load", async () => load());
  registerTrustedIpcHandler("campusos:notifications:read", async (id: string) => mutate(id, "read"));
  registerTrustedIpcHandler("campusos:notifications:unread", async (id: string) => mutate(id, "unread"));
  registerTrustedIpcHandler("campusos:notifications:handled", async (id: string) => mutate(id, "handled"));
  registerTrustedIpcHandler("campusos:notifications:read-all", async () => {
    let records = load();
    records = (records ?? []).map((entry) => entry.state === "unread" ? { ...entry, state: "read" } : entry);
    return persist(records);
  });
  registerTrustedIpcHandler("campusos:notifications:batch", async (input: { ids: string[]; state: "read" | "unread" | "handled" }) => {
  ...
  registerTrustedIpcHandler("campusos:notifications:clear-expired", async () => { const records = load(); return persist(records); });
  registerTrustedIpcHandler("campusos:notifications:clear-all", async () => {
    let records = load();
