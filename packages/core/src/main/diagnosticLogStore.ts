...
export const registerDiagnosticHandlers = (
  ...
  registerTrustedIpcHandler("campusos:diagnostics:load", async () => {
    return loadDiagnosticSnapshot();
  });
  registerTrustedIpcHandler("campusos:diagnostics:clear", async () => {
    return clearDiagnosticEntries();
  });
  registerTrustedIpcHandler("campusos:diagnostics:export", async () => {
    return exportDiagnostics();
  });
  registerTrustedIpcHandler("campusos:diagnostics:health", async () => {
    return buildHealthViewSnapshot();
  });
  registerTrustedIpcHandler("campusos:diagnostics:probe", async (sourceId: string) => {
