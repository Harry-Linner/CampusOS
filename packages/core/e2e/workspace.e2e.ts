...
test("validates the complete fixture-backed workspace at desktop and narrow widths", async ({ browserName: _browserName }, testInfo) => {
  ...
  try {
    ...
    // Navigation labels must stay on one visual line when all official modules
    // are enabled. Root-overflow checks alone missed Chinese labels wrapping.
    const navigationLines = await page.locator(".primary-navigation .nav-item span").evaluateAll((labels) => labels.map((label) => {
      const range = document.createRange();
      range.selectNodeContents(label);
      return new Set([...range.getClientRects()].map((rect) => Math.round(rect.top))).size;
    }));
    expect(navigationLines.every((lines) => lines === 1)).toBe(true);
    ...
    await expect(page.getByText("2 个待确认事项")).toBeVisible();
    ...
    const search = page.getByRole("searchbox", { name: "搜索课程、事项、资料和资讯" });
    ...
    for (const width of [820, 620]) {
      ...
      const notification = (await page.locator(".notification-trigger").boundingBox())!;
      const navigation = (await page.locator(".navigation-rail").boundingBox())!;
      const content = (await page.locator(".view-stage").boundingBox())!;
      expect(notification.y).toBeGreaterThanOrEqual(navigation.y + navigation.height);
      expect(notification.y + notification.height).toBeLessThanOrEqual(content.y);
      await expectNoRootOverflow(page);
    }
  } finally {
    await app.close();
