...
//   node scripts/visual.mjs classes <url-substring>              every class name in the DOM
//   node scripts/visual.mjs project <url-substring> <out.json>   computed-style fingerprint
//
...
function pick(pages, needle) {
  // The desk calendar is served from the same origin (localhost:5173/desk-calendar.html),
  // so a needle like "5173" matches both windows. The shortest matching URL is the least
  // specific one - the main window - which keeps "5173" meaning "the main window".
  const matches = pages.filter((page) => page.url().includes(needle)).sort((a, b) => a.url().length - b.url().length);
...
switch (command ?? "list") {
  ...
  // Every class name present in the DOM: the empirical half of a dead-CSS check.
  case "classes": {
    const [needle] = positional();
    await withBrowser(async (pages) => {
      for (const page of needle ? [pick(pages, needle)] : pages) {
        const names = await page.evaluate(() => [
          ...new Set([...document.querySelectorAll("*")].flatMap((element) => [...element.classList]))
        ].sort());
        console.log(`${shown(page.url())}\n${names.join("\n")}`);
      }
    });
    break;
  }
  // Computed-style fingerprint of every element, so a CSS change can be proven inert
  // without trusting a screenshot: same selectors, same hashes, same order.
  case "project": {
    const [needle, out] = positional();
    await withBrowser(async (pages) => {
      const page = pick(pages, needle);
      const projection = await page.evaluate(() => {
        ...
        for (const element of document.querySelectorAll("*")) {
          ...
          const selector = `${element.tagName.toLowerCase()}${element.id ? `#${element.id}` : ""}${[...element.classList]
            .map((name) => `.${name}`)
            .join("")}`;
          rows.push({ selector, hash: fnv(declarations.join(";")), declarations: declarations.length });
        }
        return {
          url: location.pathname,
          elements: rows.length,
          hash: fnv(rows.map((row) => `${row.selector}|${row.hash}`).join("\n")),
          rows
        };
      });
      const { writeFileSync } = await import("node:fs");
      writeFileSync(out, `${JSON.stringify(projection, null, 2)}\n`);
      console.log(`saved ${out}: ${projection.elements} elements, hash ${projection.hash} (${shown(page.url())})`);
    });
    break;
  }
