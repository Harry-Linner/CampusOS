...
**功能（历史对照，不再作为 CampusOS 需求）**：
- 原独立实现曾包含 `dated` / `floating` 两类任务；CampusOS 已决定不迁入 `floating` 无日期任务
- 每个任务 UUID + 修改时间戳
...
- 任务条按原独立实现的优先级排序，已完成沉底+打勾+轻微变淡；CampusOS 不迁入颜色优先级
- 格子内任务溢出时悬停滚轮局部滚动，日期数字不动
- 右键空白建任务 / 右键或双击任务编辑；原独立实现的 4 级优先级颜色不迁入 CampusOS
