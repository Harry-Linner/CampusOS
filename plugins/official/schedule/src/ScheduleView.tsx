...
/**
 * 日程筛选按「来源」分类：前三项来自上游 `calendar.events@1`（课表 / 教务考试 /
 * 学在浙大作业），最后一项来自本地任务 `tasks.local@1`。
 * 用穷尽映射而不是写死数组：将来新增 kind 时编译期即报错，不会静默消失。
 */
const EVENT_KIND_LABELS: Record<ScheduleEvent["kind"], string> = {
  course: "课程",
  exam: "考试",
  assignment: "作业",
  task: "我的日程"
...
const EVENT_KIND_ORDER: readonly ScheduleEvent["kind"][] = [
  "course",
  "exam",
  "assignment",
  "task"
...
export const ScheduleView = ({
  ...
  const omittedCalendarItems = (snapshot?.sourceStates ?? []).reduce(
    (total, state) => total + (state.omittedItemCount ?? 0),
    0
  );
  // 本轮数据源降级且未返回事件时，界面显示的是上次成功抓取的数据。
  const retainedDegradedAt = (snapshot?.sourceStates ?? [])
    .map((state) => state.retainedDegradedAt ?? null)
  ...
  const applyNavigationTarget = (target: NonNullable<PluginComponentProps["navigationTarget"]>): void => {
    if (target.entityId?.startsWith("new:")) {
      const date = target.entityId.slice(4);
      if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !Number.isFinite(Date.parse(date))) return;
      pendingNavigation.current = null;
      setSelectedDate(new Date(`${date}T09:00:00+08:00`));
      setForm(defaultTaskForm(new Date(`${date}T09:00:00+08:00`)));
      return;
    }
    const taskId = target.entityId?.startsWith("task:") ? target.entityId.slice(5) : undefined;
    const task = taskId ? tasks.find((candidate) => candidate.id === taskId) : undefined;
    if (task && task.type !== "fixedlegacy") {
      pendingNavigation.current = null;
        ...
        <section className="schedule-calendar-section" aria-label="日历">
          ...
          <div className="schedule-display-row" aria-label="日历显示选项">
            <div className="schedule-display-group">
              <span className="schedule-group-label">按来源显示</span>
              <div className="schedule-kind-filters" role="group" aria-label="按来源筛选">
          ...
          {retainedDegradedAt ? (
            <p className="muted" role="status">
              本轮数据源降级未返回事件，当前显示 {formatDateTime(retainedDegradedAt)} 抓取的数据。
            </p>
          ) : null}
          {omittedCalendarItems > 0 ? (
            <p className="muted" role="status">
              另有 {omittedCalendarItems} 项缺少可信时间，未写入日历。
            </p>
          ) : null}
