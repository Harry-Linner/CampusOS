    ...
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <label className="grid gap-1.5">
          ...
          <Input
            aria-label="模型"
            list={modelOptions.length ? "assistant-model-options" : undefined}
            value={model}
            placeholder="例如：deepseek-v4-flash"
            onChange={(event) => onModelChange(event.target.value)}
          />
          {modelOptions.length ? <datalist id="assistant-model-options">{modelOptions.map((option) => <option key={option} value={option} />)}</datalist> : null}
        </label>
      </div>
      <label className="grid gap-1.5">
