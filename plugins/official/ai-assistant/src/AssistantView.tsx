...
type BusyAction = "load-settings" | "save-settings" | "clear-settings" | "test-connection" | "discover-models" | "load-pet" | "parse" | "save-task" | null;

export const AssistantView = ({ snapshot, schedule, assistant, desktopPet, navigationTarget }: PluginComponentProps): JSX.Element => {
  ...
  const [resultSettings, setResultSettings] = useState<AiAssistantSettingsRecord | null>(null);
  const [discoveredModels, setDiscoveredModels] = useState<string[]>([]);
  const [petEnabled, setPetEnabled] = useState(false);
  const [petBusy, setPetBusy] = useState(false);
  const [petJobId, setPetJobId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    let active = true;
    void desktopPet.getState().then((state) => { if (active) setPetEnabled(state.settings.enabled); }).catch(() => undefined);
    const unsubscribe = desktopPet.subscribe((state) => { if (active) setPetEnabled(state.settings.enabled); });
    return () => { active = false; unsubscribe(); };
  }, [desktopPet]);

  useEffect(() => {
    const entityId = navigationTarget?.viewId === "ai-assistant" ? navigationTarget.entityId : undefined;
    if (!desktopPet || !entityId?.startsWith("pet:")) return;
    const id = entityId.slice(4);
    if (!id) return;
    let active = true;
    setBusy("load-pet"); setError(null); setNotice(null); setSection("message");
    void desktopPet.getJob(id).then((job) => {
      if (!active) return;
      if (!job || job.status !== "ready" || !job.result || !job.settings) throw new Error(job?.message || "这条桌宠消息尚未解析完成。");
      setMessage(job.result.sourceText);
      setPetJobId(job.id);
      setResultSettings(job.settings);
      if (job.result.intent === "academic-query") {
        setAcademicResult(job.result); setExtraction(null); setEditable([]);
      } else {
        setExtraction(job.result); setEditable(job.result.intents.map(toEditable)); setAcademicResult(null);
      }
      setNotice("已载入桌宠解析结果，请核对后再写入日程。");
    }).catch((cause: unknown) => { if (active) setError(cause instanceof Error ? cause.message : "桌宠解析结果读取失败。"); })
      .finally(() => { if (active) setBusy(null); });
    return () => { active = false; };
  }, [desktopPet, navigationTarget?.requestId, navigationTarget?.viewId, navigationTarget?.entityId]);

  const discoverModels = async (): Promise<void> => {
    if (!assistant) return;
    setBusy("discover-models"); setError(null); setNotice(null); setDiscoveredModels([]);
    try {
      const result = await assistant.discoverModels({ apiKey, provider, protocol: protocolFor(provider), baseUrl });
      setDiscoveredModels(result.models);
      setNotice(result.models.length ? `发现 ${result.models.length} 个可用模型，可在列表中选择。` : "服务没有返回可用模型；仍可手动填写模型名。");
    } catch (cause) { setError(cause instanceof Error ? cause.message : "模型列表读取失败。"); }
  ...
  const parse = async (): Promise<void> => {
    ...
    try {
      const settingsAtRequest = settings;
      const next = await assistant.parseMessage({ text: message, mode: "auto", courseNames, now: new Date().toISOString(), source: { app: "manual", sentAt: sourceSentAt ? new Date(`${sourceSentAt}:00+08:00`).toISOString() : null } });
      setResultSettings(settingsAtRequest);
      setPetJobId(null);
  ...
  const commit = async (intent: EditableIntent): Promise<void> => {
    const provenanceSettings = resultSettings ?? settings;
    if (!schedule || !provenanceSettings) return;
    if (intent.intent === "cancel" || intent.intent === "update") {
      ...
      await schedule.saveTask({ ...makeTaskInput(intent, provenanceSettings), id: target.id });
      setNotice(`已更新：${target.title}`); return;
    }
    const result = await schedule.saveTask(makeTaskInput(intent, provenanceSettings));
    ...
    <section className="page-shell assistant-page">
      ...
      {section === "settings" ? (
        <div className="space-y-6">
          <p className="max-w-2xl text-sm leading-6 text-muted-foreground">此配置仅用于 AI 助手，与早报、校园资讯的 AI 配置独立。</p>
          <section aria-labelledby="assistant-model-heading" className="space-y-4">
            ...
            <div className="settings-fields"><AssistantModelFields apiKey={apiKey} configured={settings?.configured === true} provider={provider} baseUrl={baseUrl} model={model} modelOptions={discoveredModels} onApiKeyChange={setApiKey} onProviderChange={(value) => { setProvider(value); setDiscoveredModels([]); }} onBaseUrlChange={(value) => { setBaseUrl(value); setDiscoveredModels([]); }} onModelChange={setModel} onClearKey={() => void clearSettings()} /></div>
            {discoveredModels.length ? <label className="grid gap-1.5"><span className="text-xs leading-5 text-muted-foreground">可用模型</span><select aria-label="可用模型" className="h-9 rounded-md border border-input bg-transparent px-3 text-sm" value={discoveredModels.includes(model) ? model : ""} onChange={(event) => { if (event.target.value) setModel(event.target.value); }}><option value="">请选择</option>{discoveredModels.map((candidate) => <option key={candidate} value={candidate}>{candidate}</option>)}</select></label> : null}
            ...
            <div className="settings-actions">
              ...
              <Button variant="outline" type="button" disabled={!assistant || (!apiKey.trim() && !settings?.configured) || !baseUrl.trim() || busy !== null} onClick={() => void discoverModels()}>{busy === "discover-models" ? "正在发现" : "发现模型"}</Button>
            </div>
          </section>
        ...
        <div className="assistant-layout">
          <section className="assistant-input-panel" aria-label="消息输入">
            ...
            {desktopPet ? <div className="assistant-time-context"><div className="assistant-time-copy"><span>桌面助手</span><p>把文字或图片拖给桌宠，在这里核对解析结果。</p></div><Button variant={petEnabled ? "destructive" : "outline"} type="button" disabled={busy !== null || petBusy} onClick={() => {
              setPetBusy(true);
              void (petEnabled ? desktopPet.saveSettings({ enabled: false }) : desktopPet.show())
                .then(() => desktopPet.getState())
                .then((state) => setPetEnabled(state.settings.enabled))
                .catch((cause: unknown) => setError(cause instanceof Error ? cause.message : "桌面助手切换失败。"))
                .finally(() => setPetBusy(false));
            }}>{petEnabled ? "关闭桌面助手" : "启动桌面助手"}</Button></div> : null}
          ...
          <section className="assistant-draft-panel" aria-label="提取结果">
            <header className="section-heading"><div><h2>{academicResult ? "数据问答" : "待确认事项"}</h2></div>{academicResult ? <span className="assistant-mode-badge">只读本地数据</span> : extraction ? <span className="assistant-confidence">{extraction.intents.length} 个待确认事项</span> : null}</header>
            {petJobId && desktopPet ? <div className="assistant-actions"><Button variant="ghost" type="button" onClick={() => void desktopPet.dismiss(petJobId).then(() => { setPetJobId(null); setNotice("桌宠记录已清除；当前草稿仍保留。"); }).catch((cause: unknown) => setError(cause instanceof Error ? cause.message : "桌宠记录清除失败。"))}>清除桌宠记录</Button></div> : null}
            {academicResult ? (
