...
export const Component = ({
  ...
  return (
    <section className="academic-panel" aria-label="成绩" ref={panelRef}>
      ...
      {loaded && availableRecords.length > 0 ? (
        <>
          {availableRecords.filter((record) => record.state !== "live" && record.message).map((record) => <p key={record.providerId} className="muted" role="status">{record.message}</p>)}
          {sourceMajorSummary?.state === "cache" ? <p className="meta-line">主修汇总来自缓存{sourceMajorSummary.updatedAt ? ` · 上次更新 ${new Date(sourceMajorSummary.updatedAt).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" })}` : " · 历史更新时间未知"}</p> : null}
