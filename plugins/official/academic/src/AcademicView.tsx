...
const PracticePanel = ({
  ...
  useEffect(() => {
    ...
    void capabilities
      ...
      .catch((reason: unknown) => {
        if (active) {
          setLoading(false);
          setError(reason instanceof Error && /插件|能力|绑定|认证|账号/.test(reason.message)
            ? "请在设置中连接统一身份认证，并确认学业模块已启用。"
            : "素拓记录读取失败，请重新同步。已有记录仍保留在下方。");
        }
      });
    return () => {
      active = false;
  ...
  const selectedRecord = records.find((record) => record.data) ?? records[0];
  const data = selectedRecord?.data ?? null;
  const stateLabel = selectedRecord ? { live: "已同步", cache: "上次同步记录", fallback: "部分数据可用", unavailable: "暂不可用" }[selectedRecord.state] : null;
  const message = selectedRecord?.message && /插件|能力|绑定/.test(selectedRecord.message)
    ? "请在设置中连接统一身份认证，并确认学业模块已启用。"
  ...
  const refresh = async (): Promise<void> => {
    ...
    try {
      ...
      setError("素拓同步失败，请检查连接后重试。已有记录仍保留在下方。");
    } finally {
      setRefreshing(false);
    }
  };
  return (
    <section className="academic-panel" aria-label="素质拓展实践">
      <div className="academic-panel-heading">
        ...
        <Button type="button" disabled={loading || refreshing || workspaceLoading} onClick={() => void refresh()}>{refreshing ? "正在同步" : "重新同步"}</Button>
      </div>
      {stateLabel ? <p className="meta-line">{stateLabel} · {new Date(selectedRecord!.updatedAt).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" })}</p> : null}
      {message ? <p className="muted" role={selectedRecord?.state === "unavailable" ? "alert" : "status"}>{message}</p> : null}
      {error ? (
        <p className="panel-error" role="alert">
          {error}
        </p>
      ...
      {loading ? (
        <article className="panel-card" aria-live="polite">
          <h2>正在读取</h2>
          <p className="muted">正在读取素拓实践…</p>
        </article>
      ) : !data ? (
        <p className="muted">{error || message ? "可重新同步，或前往设置检查账号连接。" : "请在设置中连接统一身份认证，再同步素拓记录。"}</p>
      ) : (
        <>
          {data.summary ? (
          ...
          {data.records.length === 0 ? <p className="muted">{data.detailsAvailable ? "本次同步成功，暂无素拓项目记录。" : "项目明细暂不可用，当前仅显示可用汇总。"}</p> : null}
          <ul className="academic-record-list">
