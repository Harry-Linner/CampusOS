...
export interface ZjuUndergraduateConnectorDependencies {
  ...
  fetchEtaTimetable?: (academicYearStart: number, semester: 1 | 2) => Promise<AcademicTimetableSession[]>;
  loadCachedTimetable: (
...
export const parseTimetableResponse = (
  ...
  if (!normalizedBody) {
    throw new Error("教务网未返回课表内容，请稍后重试。");
  }
  ...
  try {
    ...
    if (/^<!doctype\s+html(?:\s|>)/i.test(normalizedBody) || /^<html(?:\s|>)/i.test(normalizedBody)) {
      throw new Error("教务网返回了网页而非课表数据，请检查登录状态或稍后重试。", { cause: error });
    }
    throw new Error("教务网课表数据格式异常，暂时无法读取。", { cause: error });
  }
  const kbList = asRecord(payload)?.kbList;
  if (!Array.isArray(kbList)) {
    throw new Error("教务网响应缺少课程列表，暂时无法读取课表。");
  }

  const sessions = kbList
    .map((item) => parseSession(query, item))
    .filter((item): item is AcademicTimetableSession => item !== null);
  if (kbList.length > 0 && sessions.length === 0) {
    throw new Error("教务网课表响应包含条目但无法解析。");
  }
  return sessions;
};
...
const timetableTermKey = (term: Pick<AcademicTimetableTermData, "academicYearStart" | "season">): string =>
  `${term.academicYearStart}:${term.season}`;

const summarizeTimetableFailures = (
  failures: readonly { query: TimetableQuery; message: string }[]
): string => {
  const grouped = new Map<string, string[]>();
  for (const failure of failures) {
    const labels = grouped.get(failure.message) ?? [];
    labels.push(`${failure.query.academicYearStart} ${failure.query.season}`);
    grouped.set(failure.message, labels);
  }
  return [...grouped.entries()]
    .map(([message, labels]) => `${labels.length > 3 ? `${labels.slice(0, 2).join("、")}等 ${labels.length} 个学季` : labels.join("、")}：${message}`)
    .join("；");
};
...
export const createZjuUndergraduateConnector = ({
    ...
    if (result.ok) {
      try {
        ...
        if (result.majorBody) {
          data = {
            ...data,
            majorSummary: { ...parseMajorGradeSummaryResponse(result.majorBody), state: "live", updatedAt }
          };
        } else {
          const cached = await loadCachedGrades(proof.studentId);
          const cachedSummary = cached?.majorSummary;
          if (cachedSummary) data = { ...data, majorSummary: { ...cachedSummary, state: "cache" } };
        }
        const state = result.majorBody ? "live" as const : "fallback" as const;
        await publish({
          ...
          updatedAt,
          data,
          ...(!result.majorBody ? { message: data.majorSummary
            ? "成绩明细已更新；主修成绩暂不可用，当前显示上次主修汇总。"
            : "成绩明细已更新；主修成绩暂不可用，尚无可用主修汇总。" } : {})
        });
  ...
  const refresh = async (): Promise<ConnectorRefreshResult> => {
    ...
    // 缓存优先（Cache-First）：启动时在发起耗时网络请求前，先行发布本地已有的课表、考试、成绩、素拓及课程目录缓存
    const [cachedTimetableInitial, cachedExamsInitial, cachedGradesInitial, cachedPracticeInitial, cachedCatalogInitial] =
      await Promise.all([
        ...
        loadCachedPractice?.(proof.studentId).catch(() => null) ?? Promise.resolve(null),
    ...
    if (cachedPracticeInitial) {
      await publish({
        capability: "practice.records@1",
        accountId: proof.studentId,
        state: "cache",
        updatedAt,
        data: cachedPracticeInitial,
        message: "已预载本地素拓缓存，正在后台检查更新…"
      });
    }
    ...
    const terms: AcademicTimetableTermData[] = queries.map((query) => {
      ...
      try {
        const sessions = parseTimetableResponse(query, result.body);
        return {
          ...query,
          state: "live" as const,
          sessions,
          ...(sessions.length === 0 ? { message: "教务网课表返回空数据。" } : {})
        };
      } catch (error) {
        return {
          ...query,
    ...
    if (fetchEtaTimetable) {
      for (const [index, term] of terms.entries()) {
        if ((term.state === "live" && term.sessions.length > 0) || term.academicYearStart > currentAcademicYear) continue;
        const semester = term.season.startsWith("1|") ? 1 : 2;
        const key = `${term.academicYearStart}:${semester}`;
        let pending = etaSemesters.get(key);
        if (!pending) {
          pending = fetchEtaTimetable(term.academicYearStart, semester);
          etaSemesters.set(key, pending);
        }
        try {
          const sessions = await pending;
          if (sessions.length > 0) {
            etaUsed = true;
            terms[index] = { ...term, state: "live", source: "eta", sessions, message: "zdbk 未提供可用课表，已切换 ETA 实时来源。" };
          } else {
            term.message = `${term.message ?? "zdbk 未提供可用课表。"} ETA 未返回该学期课程。`;
          }
        } catch (error) {
          const code = asRecord(error)?.code;
          const detail = typeof code === "string" && /^[a-z-]+$/.test(code) ? `（${code}）` : "";
          term.message = `${term.message ?? "zdbk 未提供可用课表。"} ETA 请求失败${detail}。`;
        }
      }
    }
    const needsTimetableCache = terms.some((term) =>
      term.state === "unavailable" || (term.state === "live" && term.sessions.length === 0)
    );
    const cachedTimetable = needsTimetableCache
      ? await loadCachedTimetable(proof.studentId)
      : null;
    const cachedTerms = new Map(
      (cachedTimetable?.terms ?? [])
        .filter((term) => term.state !== "unavailable")
    ...
    const effectiveTerms = terms.map((term) => {
          ...
          const message = term.message ?? "教务网课表返回空数据。";
          cacheFallbackFailures.push({ query: term, message });
          return {
            ...cachedTerm,
    ...
    const allTimetableFailures = [
      ...effectiveTerms.flatMap(term => term.state === "unavailable" && term.message ? [{ query: term, message: term.message }] : []),
      ...cacheFallbackFailures
    ];
    const hasLiveTerm = effectiveTerms.some(term => term.state === "live");
    ...
    if (hasLiveTerm) {
      ...
      timetableStatus = effectiveTerms.every((term) => term.state === "live") && !etaUsed
        ? "live"
        : hasAvailableTerm
          ? (hasLiveTerm ? "fallback" : "cache")
          : "unavailable";
      if (allTimetableFailures.length > 0) {
        timetableMessage = `课表部分学季不可用${cacheFallbackFailures.length > 0 ? "，已保留同学期缓存" : ""}。${summarizeTimetableFailures(allTimetableFailures)}`;
      }
      if (etaUsed) timetableMessage = `zdbk 未提供可用课表的学季已切换 ETA 实时来源。${timetableMessage ?? ""}`;
      await publish({
        capability: "academic.timetable@1",
        accountId: proof.studentId,
        ...
        ...(timetableMessage ? { message: timetableMessage } : {})
      });
    } else {
      const hasAvailableTerm = effectiveTerms.some((term) => term.state === "cache");
      if (hasAvailableTerm) {
        timetableMessage = allTimetableFailures.length > 0
          ? `实时课表不可用，已使用同学期缓存。${summarizeTimetableFailures(allTimetableFailures)}`
          : "实时课表不可用，已使用同学期缓存。";
        timetableStatus = effectiveTerms.every((term) => term.state === "cache")
          ? "cache"
          : "fallback";
        await publish({
          capability: "academic.timetable@1",
        ...
        timetableMessage = allTimetableFailures.length > 0
          ? `教务网课表当前不可用，且没有可用缓存。${summarizeTimetableFailures(allTimetableFailures)}`
          : "教务网课表当前不可用，且没有可用缓存。";
        await publish({
          capability: "academic.timetable@1",
          accountId: proof.studentId,
    ...
    const status = moduleStates.every((state) => state === "live")
      ...
      : moduleStates.every((state) => state === "unavailable")
        ...
        : moduleStates.some((state) => state === "live" || state === "fallback")
          ? "fallback"
          : "cache";
    const message = timetableMessage ?? (status === "fallback"
