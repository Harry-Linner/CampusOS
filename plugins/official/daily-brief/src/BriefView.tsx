    ...
    <section className="mx-auto w-full max-w-5xl px-1 pb-10 sm:px-2">
      ...
      {tab === "settings" ? (brief ? <InterestSettings brief={brief} /> : <Alert variant="destructive"><AlertCircle className="size-4" /><AlertTitle>桥接不可用</AlertTitle><AlertDescription>早报桥接不可用，请重启 CampusOS。</AlertDescription></Alert>) : (
        <div className="space-y-6">
          ...
          {loading ? <div className="space-y-3" role="status" aria-live="polite"><div className="flex items-center gap-2 text-sm text-muted-foreground"><RefreshCw className="size-4 animate-spin" aria-hidden="true" /><span>{statusLabel[state.status]}，旧内容仍可阅读</span></div>{!snapshot ? [0, 1].map((index) => <div key={index} className="space-y-3"><Skeleton className="h-5 w-36" /><Skeleton className="h-4 w-full" /><Skeleton className="h-4 w-2/3" /></div>) : null}</div> : null}
          ...
          {showEmpty ? <div className="flex flex-col items-center gap-4 px-6 py-16 text-center"><div className="flex size-12 items-center justify-center rounded-full bg-primary/10 text-primary"><Newspaper className="size-5" aria-hidden="true" /></div><div><h2 className="font-semibold leading-7">今天还没有早报</h2><p className="mt-1 max-w-md text-sm leading-6 text-muted-foreground">点击刷新抓取公开资讯。生成摘要需要在设置页的「AI 连接」中配置早报自己的服务商与模型。</p></div><Button onClick={() => void refresh()} disabled={!brief}><RefreshCw aria-hidden="true" />生成今日早报</Button></div> : null}

          {snapshot && !hasContent && !loading ? <p className="py-14 text-center text-sm text-muted-foreground">{snapshot.note ?? "今日暂无新内容。"}</p> : null}

          {snapshot && hasContent ? <div>{snapshot.sections.map((section, sectionIndex) => <section key={`${section.interest}-${sectionIndex}`} aria-labelledby={`brief-section-${sectionIndex}-heading`} className="border-t border-border/60 pt-5 pb-2"><div className="flex items-center justify-between gap-3"><div><h2 id={`brief-section-${sectionIndex}-heading`} className="text-lg font-semibold leading-7">{section.interest}</h2><p className="mt-0.5 text-xs leading-5 text-muted-foreground">{section.items.length} 条摘要</p></div><span className="font-mono text-xs text-muted-foreground">{String(sectionIndex + 1).padStart(2, "0")}</span></div><div className="divide-y divide-border/60">{section.items.map((item) => <article key={item.fingerprint} className="py-4"><h3 className="text-base font-semibold leading-7 text-foreground">{item.titleZh}</h3><p className="mt-2 max-w-3xl text-sm leading-7 text-muted-foreground">{item.summary}</p><div className="mt-3 flex flex-wrap items-center gap-2 text-xs leading-5 text-muted-foreground"><Badge variant="secondary">{item.sourceLabel}</Badge>{item.relevance ? <span>{item.relevance}</span> : null}</div>{expanded[item.fingerprint] ? <p className="mt-3 border-l-2 border-primary/30 pl-3 text-xs leading-5 text-muted-foreground">原标题：{item.originalTitle}</p> : null}<div className="mt-4 flex flex-wrap items-center gap-2"><Button size="sm" variant="outline" onClick={() => openOriginal(item)}><ExternalLink className="size-3.5" aria-hidden="true" />阅读原文</Button><Button size="sm" variant="ghost" onClick={() => setExpanded((current) => ({ ...current, [item.fingerprint]: !current[item.fingerprint] }))}>{expanded[item.fingerprint] ? "收起详情" : "查看详情"}</Button></div></article>)}</div></section>)}</div> : null}
