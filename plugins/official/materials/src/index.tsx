...
const findMaterialDownload = (
  ...
  downloads.find(
    (download) =>
      ...
      download.semester === material.semester &&
      download.sourceUrl === material.downloadUrl
  ) ?? null;
...
export const Component = ({
  ...
  const verifyFile = async (
    download: CampusDownloadTask,
    material?: CampusMaterialRecord
  ): Promise<void> => {
    if (!downloads?.verify) return;
    setBusyId(download.id);
    setActionError(null);
    setNotice(null);
    try {
      const result = await downloads.verify(download.id);
      if (result.status === "verified") setNotice(`“${download.title}”本地文件校验通过。`);
      else if (material?.downloadUrl) {
        await downloads.cancel(download.id);
        await downloads.enqueue(toDownloadRequest(material));
        await onRefresh();
        setNotice(
          result.status === "missing"
            ? `“${download.title}”已不在原保存位置，已重新加入下载队列。`
            : `“${download.title}”大小与下载记录不一致，已重新加入下载队列。`
        );
      } else if (result.status === "missing") {
        setActionError(`“${download.title}”已不在原保存位置，请从课程资料中重新下载。`);
      } else {
        setActionError(`“${download.title}”大小与下载记录不一致，请从课程资料中重新下载。`);
      }
    } catch (error) {
      setActionError(error instanceof Error ? error.message : "本地文件校验失败。");
    } finally {
      setBusyId(null);
    }
  };
                      ...
                      <li key={material.id} data-search-id={material.id} className="materials-file-row">
                        ...
                        {material.downloadUrl && downloads ? (
                          <Button
                            variant="ghost"
                            type="button"
                            disabled={busyId === material.id || busyId === download?.id}
                            onClick={() =>
                              void (download?.status === "ready" && downloads.verify
                                ? verifyFile(download, material)
                                : runAction(material.id, () => downloads.enqueue(toDownloadRequest(material))))
                            }
                          >
                            {busyId === material.id
                              ? "处理中"
        ...
        <section className="materials-downloads" aria-label="下载队列">
          <header className="section-heading">
            ...
            <div className="section-heading-actions">
              ...
              {downloads?.clearHistory && snapshot.downloads.some((item) => item.status === "ready" || item.status === "failed") ? (
                <Button variant="ghost" type="button" onClick={() => void runFileAction("clear-history", async () => {
                  const count = await downloads.clearHistory!();
                  setNotice(count > 0 ? `已清除 ${count} 条完成或失败记录。` : "没有可清除的记录。");
                  await onRefresh();
                })}>清除历史</Button>
              ) : null}
            </div>
                ...
                <li key={download.id} className="materials-download-row">
                  ...
                  {downloads && download.status === "ready" ? (
                    <div className="inline-actions">
                      ...
                      {downloads.verify ? <Button
                        variant="ghost"
                        type="button"
                        disabled={busyId === download.id}
                        onClick={() => void verifyFile(download)}
                      >校验本地文件</Button> : null}
                    </div>
                  ) : downloads ? (
                    <div className="inline-actions">
