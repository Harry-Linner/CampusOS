...
import { CampusFeedOnboarding } from "./CampusFeedOnboarding";
import { CampusFeedSubscriptions } from "./CampusFeedSubscriptions";
...
export const CampusFeedView = (props: PluginComponentProps): JSX.Element => {
  ...
  const [navigationItems, setNavigationItems] = useState<FeedItemRecord[]>([]);
  ...
  const markAllRead = useCallback(async (): Promise<void> => {
    ...
    const activeIds = new Set(snapshot.sources.filter((source) => source.enabled).map((source) => source.id));
    const unread = snapshot.items
      .filter((item) => activeIds.has(item.sourceId) && item.state === "new")
      .map((item) => item.id);
  ...
  // Paused subscriptions retain their local history for lossless restore, but
  // hidden rows must not affect visible unread counts or bulk read actions.
  const items = (snapshot?.items ?? []).filter((item) => enabledSourceIds.has(item.sourceId));
  ...
  const navigationSourceIds = new Set(navigationItems.map(item => item.sourceId));
  const visibleSources = navigationIds
    ? (snapshot?.catalog ?? sources).filter(source => navigationSourceIds.has(source.id)).map(source => sources.find(s=>s.id===source.id) ?? source)
    : enabledSources.filter(matchesSource);
  ...
  useEffect(() => {
    ...
    if (
      !feed || target?.viewId !== "campus-feed" || (!target.entityId && !target.entityIds?.length) ||
    ...
    setNavigationItems([]);
    setNavigationIsBatch(false);
    setHighlightedItemId(null);
    setNavigationLoading(true);
    void feed.getItems(targetIds).then(matched => {
      if (!active) return;
      if (!matched.length) {
        toast.error("这条历史资讯已不存在。");
        return;
      }
    ...
    setOnlyUnread(false);
    setViewMode("grouped");
    setNavigationIds(new Set(matched.map(entry=>entry.id)));
    setNavigationItems(matched.map(entry=>target.entityId ? { ...entry, state: "read" } : entry));
    setCollapsedSources(current => new Set([...current].filter(id=>!matched.some(entry=>entry.sourceId===id))));
    setNavigationIsBatch(!target.entityId);
    setHighlightedItemId(item.id);
    if (target.entityId) void feed.markRead([item.id]).catch(() => toast.error("未能保存已读状态。"));
    requestAnimationFrame(() => {
      document.querySelector(`[data-feed-item-id="${item.id}"]`)?.scrollIntoView({ block: "center" });
    });
    }).catch(() => { if (active) toast.error("无法读取历史资讯，请重新搜索后打开。"); })
      .finally(() => { if (active) setNavigationLoading(false); });
    return () => {
      active = false;
      handledNavigationRef.current = null;
    };
  }, [feed, props.navigationTarget]);
  const resetFilters = (): void => {
    setNavigationItems([]); setHighlightedItemId(null);
  ...
  return (
    <section className="mx-auto w-full max-w-5xl px-1 pb-10 sm:px-2">
      ...
      {!feed ? (
        ...
        <div className="settings-panel">{snapshot && <CampusFeedSubscriptions snapshot={snapshot} feed={feed} onPreferences={() => setPreferencesOpen(true)} />}</div>
      ) : (
        <div className="space-y-6">
          ...
          {navigationIds ? (
            <Alert>
              <AlertTitle>{navigationIsBatch ? "本次提醒的资讯" : "已定位历史资讯"}</AlertTitle>
              <AlertDescription className="flex flex-wrap items-center justify-between gap-3">
                <span>{navigationIsBatch ? `正在查看本次批量提醒中的 ${visibleItems.length} 条资讯。` : "来源下仅展示此次定位的条目。"}</span>
                <Button size="sm" variant="outline" onClick={resetFilters}>返回每源最近 50 条</Button>
              </AlertDescription>
            </Alert>
          ) : null}
          ...
          <p className="text-xs text-muted-foreground">普通浏览每源显示最近 50 条；按 Ctrl+F 搜索全部已保存的历史列表信息。</p>
          {navigationLoading ? <p role="status">正在读取历史资讯…</p> : null}
          {sources.length === 0 && !navigationIds ? (
            <div className="flex flex-col items-center gap-4 px-6 py-16 text-center">
              ...
              <div>
                ...
                <p className="mt-1 max-w-md text-sm leading-6 text-muted-foreground">选择你关心的官网和栏目，资讯就会出现在这里。离线时仍可管理订阅。</p>
              </div>
              <Button onClick={() => setTab("sources")}>发现信息源</Button>
            </div>
            ...
            <div>
              {viewMode === "all" ? (
                ...
                visibleSources.map((source) => {
                  ...
                  return (
                    <section key={source.id} aria-labelledby={`feed-source-${source.id}-heading`} className="pt-5 pb-2">
          ...
          {scheduleCandidates.length === 0 ? (
            ...
            <div className="divide-y divide-border/60">
              {scheduleCandidates.map((candidate, index) => (
                <div key={`${candidate.itemId}-${index}`} className="flex items-start justify-between gap-3 py-3"><div>
                  <div className="flex flex-wrap items-center gap-2">
