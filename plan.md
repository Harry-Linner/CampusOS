...
- SQLite `DatabaseService` 迁移链已推进至 v14：工作区快照、官方 capability provenance、下载队列、日程任务、校园资讯刷新基线与通知关键词、桌历状态与个性化以及通知内容写入同一数据库，旧 JSON 仅作一次性导入。通知写入和备份恢复使用事务；自动排程历史表由 migration 10 删除，migration 3 保留用于连续升级。Electron 依赖通过 `rebuild:electron` 重新编译 native binding。
...
- [x] **收尾下载队列与通知/资讯改动。** 下载保留独立的临时提醒，不进入通知中心；通知中心使用 `unread/read/handled/expired`、保留最近 500 条并优先保护未读；校园资讯以来源通知开关和全局关键词筛选新增/标题摘要更新，资讯页保留全部内容，批次只弹一次桌面汇总并可定位本批列表。提示音素材已记录来源和许可边界。
...
- [ ] **完成桌历、重复事件和校历契约。** SQLite 状态、上游个性化、统一校历、从属恢复及重复事件的已确认回归已修复；桌历始终贴底。剩余系统实机门禁为 Wallpaper Engine 运行与手动 Win+D；见 `docs/specs/desk-calendar-and-recurrence.md`。未因单元测试通过而关闭该门禁。
...
- [x] Use Celechron's first returned repeated-course attempt for academic GPA and earned credits.
- [x] Keep the independent highest-score projection out of CampusOS settings and renderer IPC.
- [x] Cover repeated attempts and the fixed first-attempt behavior with tests; retain the legacy SQLite table only for non-destructive compatibility.
...
### Summer short-term (短学期) fallback (2026-08-16)

- [x] ZJU 短学期课程挂在教务系统 `xqm=2|夏` 学季（真实校历 4/27–7/05），
  抓取层本就覆盖四季，但投影层在暑假会直接选下一学年秋冬而跳过它。
- [x] `selectAcademicSemesterWindow` 按 Celechron 的 14 天边界回落：春夏学期结束后 14 天内
  优先投影刚结束的春夏学期（含 2|夏 短学期课程），之后才回落到下一学年秋冬；
...
- [x] 新增 Core 生命周期偏好存储、开机自启 IPC、托盘与单实例恢复主窗口。
...
- CampusOS 生命周期已统一：主窗口关闭支持每次询问、隐藏到托盘或退出；支持“设为默认，以后不再询问”；单实例恢复、托盘视图切换和日程插件停用联动已经落地。
