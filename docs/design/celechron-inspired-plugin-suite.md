...
| `schedule` | 日程 | 月历、周视图、日程、日视图、接下来、DDL、个人任务、固定/重复日程、自动排程、系统日历同步、iCal 导出 |
...
| 五分制、4.3、4.0、百分制、主修均绩、加权试算 | 学业插件 | 沿用 Celechron `Grade`、GPA helper 和 weighted GPA 行为 |
...
| `planner.schedule@1` | 工作段、休息段、不可行原因和计划版本 | 日程 |
...
- 日历：月、周、日程和日视图。
- 接下来：未来 48 小时内课程、固定日程和计划段的连续时间流。
- 任务：截止任务、固定/重复日程、预计/已用时间、暂停、继续、完成和删除。
- 规划：可用时间、工作/休息段、重新排程和不可行原因。
...
任务类型、状态、重复和排程算法以 Celechron 1.3.0 为唯一业务来源：
...
- deadline 预计用时至少一分钟。
- 规划先扣除课程和阻塞日程，再按任务剩余时长填充可用时间。
- 可拆分任务每段最长为工作时长，段间插入休息；不可拆分任务必须连续完成。
- 目标休息不可行时二分搜索最大可行休息时间；仍不可行则返回空计划和可解释原因。
- 进行中的计划段按墙上时间累计任务耗时，并成对持久化任务和计划进度。
...
课程、考试、DDL 和用户日程统一投影为 `calendar.events@1`。自动规划结果单独提供 `planner.schedule@1`，由日程插件在读取时叠加，避免事件集合和规划器形成激活循环。
