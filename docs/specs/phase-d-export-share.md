...
- [x] 三视图均可导出 Markdown，内容与视图一致（表格/列表/时间保留）
- [ ] PNG 导出内容与视图一致（截图覆盖目标区域，宽度≥内容宽度；当前缺桌面截图验收）
- [ ] 导出按钮遵循既有视图工具区样式；无装饰框/无生造位移/窄屏不溢出（ai-frontend-lessons）
- [x] 保存走正式 IPC；失败有明确错误提示；不伪造成功
- [x] 勾选粒度：视图级"导出当前视图"；列表类视图支持勾选若干条目后导出所选（见 §4）
- [x] 测试：Markdown 序列化（纯函数）、保存 IPC、视图按钮交互（含勾选）
