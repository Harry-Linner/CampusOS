...
> **实现检查点（2026-07-29）：** 内置官方插件已迁移到 Manifest v2，并具备主进程持久化授权、能力解析、headless 生命周期、刷新协调和 provenance。本科与研究生教务连接器通过核心托管的不可导出 Session/token 接入课表、考试和成绩；学在浙大连接器发布作业和课件目录，主进程按 60–120 秒随机间隔刷新全部课程并托管认证下载。插件不能读取密码、Cookie 或 token，renderer capability 读取按 manifest binding 和当前验证账号隔离。2026-07-29 本科真实账号已脱敏通过统一认证、教务、素拓、作业、完整课件目录和一份授权私有课件的认证下载及字节校验；多设备和研究生真实账号仍待验收。
...
| Core 内部服务 | 为模块提供事件、排程、搜索、通知和导出，不属于插件 | 事件投影、全局搜索、iCal |
...
- **包签名：** `.campusmod` 已验证 Ed25519 签名及规范内容载荷；信任目录和签名密钥连续性后续引入
...
| 6 | .campusmod 签名信任目录 | 🟢 低 | 已完成本地 Ed25519 验证，后续补开发者身份与密钥轮换 |
