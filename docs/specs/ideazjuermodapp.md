...
> **实现检查点（2026-07-28）：** 内置官方插件已迁移到 Manifest v2，并具备主进程持久化授权、能力解析、headless 生命周期、刷新协调和 provenance。本科与研究生教务连接器通过核心托管的不可导出 Session/token 接入课表、考试和成绩；学在浙大连接器发布作业和课件目录，主进程按 60–120 秒随机间隔刷新全部课程并托管认证下载。插件不能读取密码、Cookie 或 token，renderer capability 读取按 manifest binding 和当前验证账号隔离。2026-07-28 本科真实账号已脱敏通过统一认证、教务、素拓、作业和完整课件目录；认证下载自动化协议通过，私有课件实体、多设备和研究生真实账号仍待验收。
