...
> **范围口径（2026-09 文档清理）：** 上述冻结与当前开发并行关系以 [plan.md](../plan.md) 的 Current Development Workboard 为准——workboard 已立项项（桌面日历收尾、通知中心、下载队列、campus-feed 等）不受本冻结限制，但都不构成 Release Go/No-Go 证据。官方用户模块现为五个（含 校园资讯）；campus-feed 的真实抓取依赖外部网络可达性，Release 前至少完成一次默认源真实抓取手验并记录脱敏结果，不设自动化门槛。
...
> 本文此前逐字重复的"2026-08-16 决策同步 / 实现状态同步"大段已移除；相关口径见 CONTEXT.md 对应章节（运行时决策单一事实源）。
