...
- 当前实现按 Celechron 1.3.0 的已验证行为做 TypeScript 机械迁入；没有复制其 GPL-3.0 源码文本，来源、适配边界和偏离原因记录在下方对照章节。
...
### Celechron 1.3.0 本科课表对照

本科教务请求严格对照 `.tmp/celechron-1.3.0/lib/http/zjuServices/zdbk.dart`：`JSESSIONID` 与 `route` 成对携带；课表、考试和成绩请求使用其 Chrome `User-Agent`、`Connection: close`、XHR `Accept`、`Referer` 和 `X-Requested-With`；未取得验证码时课表字段 `captcha_value` 传字符串 `null`。Celechron 在收到 `captcha_error` 时弹出原生验证码输入；CampusOS 目前没有独立且安全的验证码交互通道，因此在同一条件下 fail closed 为 `interactive-verification-required`，不伪造验证码、不静默重试，也不把失败改写为成功。

### Celechron 1.3.0 学在浙大对照
