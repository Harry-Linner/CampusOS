...
### 5.1 用户决议（2026-08-24 已拍板）

- [x] **第一类（工程内核与可靠性）全选**：Phase A、B、K1–K5
- [x] **第二类（界面与体验优化）**：
  - [x] Phase D（导出与分享）、Phase E（侧栏子 Tab + 懒加载）入选
  - [ ] Phase C（数据来源诊断面板）**不入选** —— 决议理由：数据流水/事件流可视化为冗余工程，不做
  - [ ] （注）D/E 实施前提：**必须保持审美纪律**，禁止无意义装饰框、字符重叠、生造位移/手动对齐、magic number 拼缝等低级错误；开工前先完成"AI 制作前端经验教训"搜罗并沉淀为本地规避清单（见 docs/research/ai-frontend-lessons.md）
- [ ] **第三类（新能力/新插件）逐项待议**：F、G、H、I、J —— 每一项先与用户商量功能设计与使用方式，全部议定后再统一开工；未议定前不立项
  - [x] Phase G（分层本地知识）**不入选** —— 决议理由：用户评估"资料搜索/已读收藏/AI 可查"为低价值，痛点不成立，跳过（2026-08-24）
  - [x] Phase H（对话式学业分析）**入选**（2026-08-24）—— 细化决议：
    - 与现有 AI 助手**合并实现**，AI 助手**自动切换模式**：识别学业类意图时自动切到"本地数据问答"（只读课表/成绩/作业），其余维持通用对话
    - 边界不变：只读、最小上下文、证据引用、prompt-injection 沿用既有测试矩阵
    - 后续方向（暂缓，未立项）：**桌面宠物（AI 小秘书交互面）** —— 用户与桌宠直接交互，桌宠作为 AI 助手的快捷入口；与 H 共享同一数据问答内核
  - [x] Phase F（常驻通知/推荐面板）**入选**（2026-08-24）—— 细化决议：
    - **不写新插件**：功能落在现有"校园资讯"（campus feed）模块内，作为其新能力
    - 常驻形态：**在校园日历里开辟板块**，通知消息在日历中浮现（数据归属校园资讯，展示位置在日历）
    - **AI 提取写进日程：必做**（勾选通知 → ADR-0004 受控提取 {标题,开始,结束,地点} → 用户确认 → ADR-0003 写入日程）
    - 推荐与画像：**后置**（待网站/订阅源配置齐全后再议）
    - 附带事项：现有校园日历 UI/交互需**重构**（用户判定现状不佳），前端以本机 DeskToDo（D:\DeskToDo）为参考照抄其 UI/交互写法，降低实现成本
    - 依赖修正：原"依赖 Phase C/G"均已砍 → F 实际独立，仅对齐 ADR-0003/0004
  - [x] Phase I（应用内插件市场）**排期极靠后，当前不考虑**（2026-08-24）—— 决议理由：用户暂无力顾及，先不进入视野；依赖 Phase B 的前提记录保留
  - [x] Phase J（通道通知）**后置，当前不做**（2026-08-24）—— 决议理由：调研未见钉钉接入的方便方式，接入成本不划算；待出现顺手方案（官方 API/机器人流程简化）再议；桌面提醒验收仍是前置条件

### 第三类全部议定（2026-08-24）：F ✅ / H ✅ / G ❌ / I ⏸ / J ⏸ —— 满足"议定后再开工"前提
- [ ] 开工顺序：先完成前端规避点沉淀 → 逐项议定第三类 → 按 Feature Completion 纪律立项实施
