...
3. **Tray lifetime.** While CampusOS is running, the tray provides `打开 CampusOS`, `显示/隐藏桌面日历`, the desktop-calendar view choices, `立即同步`, and `退出 CampusOS`. Desktop-calendar items are disabled or hidden when the Schedule plugin is disabled.
